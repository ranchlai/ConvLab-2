...
def slot_translate(domain, slot):
    ...
    try:
        if slot.lower() in vocab_dict["slot_set"]:
            ...
            capitalize(domain) in REF_USR_DA
            and slot.lower() in REF_USR_DA[capitalize(domain)]
        ):
            return vocab_dict["slot_set"][
                REF_USR_DA[capitalize(domain)][slot.lower()].lower()
            ]
        elif (
            capitalize(domain) in REF_SYS_DA
            and capitalize(slot) in REF_SYS_DA[capitalize(domain)]
        ):
            return vocab_dict["slot_set"][
                REF_SYS_DA[capitalize(domain)][capitalize(slot)].lower()
            ]
...
def value_translate(domain, slot, value):
    ...
    if (
        slot not in vocab_dict["value_set"][domain]
        and slot.lower() in REF_USR_DA[capitalize(domain)]
    ):
    ...
    if value not in vocab_dict["value_set"][domain][slot]:
        if "|" in value:
            return "或".join(
                [value_translate(domain, slot, v) for v in value.split("|")]
            )
...
def ontology_translate(typ, *args):
    assert typ in [
        ...
        "slot",
        "value",
    ], "Function translate() requires 1st argument: domain|slot|value."
    if typ == "intent":
