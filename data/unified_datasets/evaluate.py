...
def check_ontology(name):
        ...
        for slot_name, slot in domain["slots"].items():
            ...
            if slot["is_categorical"]:
                ...
                for value in special_values:
                    ...
                    ), f"ONTOLOGY\tspecial value `{value}` should not present in possible values"
    ...
    for bda in ontology["binary_dialogue_act"]:
        assert (
            bda["intent"] is None or bda["intent"] in ontology["intents"]
        ), f"ONTOLOGY\tintent undefined intent in binary dialog act: {bda}"
...
def check_data(name, ontology):
    ...
    stat = {split: {key: 0 for key in stat_keys} for split in splits}
    ...
    for dialogue in data:
        ...
        assert (
            dialogue["dataset"] == name
        ), f'{dialogue_id}\tinconsistent dataset name: {dialogue["dataset"]}'
        ...
        assert isinstance(
            cur_domains, list
        ), f"{dialogue_id}\t`domains` is expected to be list type, "
        assert len(set(cur_domains)) == len(
            cur_domains
        ), f"{dialogue_id}\trepeated domains"
        cur_stat["domains"] += len(cur_domains)
        cur_domains = set(cur_domains)
        for domain_name in cur_domains:
            assert (
                domain_name in ontology["domains"]
            ), f"{dialogue_id}\tundefined current domain: {domain_name}"
        ...
        if ontology["state"]:
            ...
            cur_state = {
                domain_name: deepcopy(ontology["state"][domain_name])
                for domain_name in cur_domains
            }
        # check dialog act
        for turn_id, turn in enumerate(turns):
            ...
            if turn_id > 0:
                ...
                ), f"{dialogue_id}:{turn_id}\tuser and system should speak alternatively"
            ...
            def check_dsv(domain_name, slot_name, value, categorical, prefix):
                assert (
                    domain_name in cur_domains or domain_name == "booking"
                ), f"{prefix}\t{domain_name} not presented in current domains"
                domain = ontology["domains"][domain_name]
                assert (
                    slot_name in domain["slots"]
                ), f"{prefix}\t{slot_name} not presented in domain {domain_name}"
                ...
                if categorical:
                    ...
                    assert not slot[
                        "is_categorical"
                    ], f"{prefix}\t{domain_name}-{slot_name} is not non-categorical"

            def check_da(da, categorical):
                assert (
                    da["intent"] in ontology["intents"]
            ...
            for da in dialogue_acts["binary"]:
                assert (
                    tuple(da.values()) in ontology["bda_set"]
                ), f"{dialogue_id}:{turn_id}\tbinary dialog act {da} not present in ontology"
                # do not check domain-slot-value in binary dialogue acts

            if turn["speaker"] == "user":
                assert (
                    "state" in turn and "state_update" in turn
                ), f"{dialogue_id}:{turn_id}\tstate and state_update must present in user's role"
                ...
                def apply_update(update, categorical):
                    ...
                    check_dsv(
                        ...
                        value,
                        categorical,
                        f"{dialogue_id}:{turn_id}:state_update",
                    )
                ...
                if ontology["state"]:
                    ...
                    for update in state_update["non-categorical"]:
                        ...
                        if value not in special_values:
                            ...
                            if "utt_idx" in update:
                                if (
                                    turns[update["utt_idx"]]["utterance"][
                    ...
                    ), f"{dialogue_id}:{turn_id}:state_update incorrect state or state update calculation"

            else:
                assert (
                    "state" not in turn or "state_update" in turn
                ), f"{dialogue_id}:{turn_id}\tstate or state_update cannot present in system's role"

        assert (
            turns[-1]["speaker"] == "user"
    ...
    for split in splits + ["all"]:
        ...
        if cur_stat["dialogues"]:
            cur_stat["avg_utt"] = round(
                cur_stat["utterances"] / cur_stat["dialogues"], 2
            )
            cur_stat["avg_tokens"] = round(
                cur_stat["tokens"] / cur_stat["utterances"], 2
            )
            cur_stat["avg_domains"] = round(
                cur_stat.pop("domains") / cur_stat["dialogues"], 2
            )
        else:
            del stat[split]
