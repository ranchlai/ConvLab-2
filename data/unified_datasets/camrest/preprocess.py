...
def convert_da(utt, da, all_intent, all_binary_das):
    converted_da = {"binary": [], "categorical": [], "non-categorical": []}
    ...
    for s, v in diff_state.items():
        ...
        if s in cat_slot_values:
            ...
            prev_sys_da = (
                []
                if len(prev_turns) == 0
                else prev_turns[-1]["dialogue_act"]["non-categorical"]
            )
            for _sys_da in prev_sys_da:
                if (
                    _sys_da["slot"] == s
                    and _sys_da["value"] == v
                    and "start" in _sys_da
                ):
            ...
            if not found:
                state_update["non-categorical"].append(
                    {"domain": "restaurant", "slot": s, "value": v}
                )
                ...
                for turn in dialogue:
                    ...
                    sys_turn = {
                        ...
                        "dialogue_act": convert_da(
                            sys_text, sys_da, all_intent, all_binary_das
                        ),
                    }
