...
state_cat_slot_value_dict = {
    ...
    "hotel-internet": {"yes": 1841, "no": 79, "free": 2},
    "hotel-area": {
        "east": 416,
        "north": 717,
        "centre": 538,
...
def convert_da(utt, da_dict, binary_ont, intent_ont, did, tid, da_cat_slot_values):
    ...
    :return:
    """
    converted_da = {"categorical": [], "non-categorical": [], "binary": []}
    ...
    for Domain_Act, S, v in da_dict:
        ...
        if (
            reformated_domain_slot in slot_to_type
            and slot_to_type[reformated_domain_slot] == "cat"
        ):
            origin_v = v
            v = v.lower()
        ...
        for slot in diff_state[domain]:
            ...
            for value in value_list:
                    ...
                    if reformated_domain_slot in state_cat_slot_value_dict and (
                        ...
                        if domain + "-" + slot not in da_cat_slot_values:
                            da_cat_slot_values[domain + "-" + slot] = [
                                diff_state[domain][slot]
                            ]
                        da_cat_slot_values[domain + "-" + slot].append(
                            diff_state[domain][slot]
                        )
                ...
                for _Domain_Act, _Slot, _value in coref_dict:
                    ...
                    _slot = _Slot.lower()
                    _coref_value = coref_dict[(_Domain_Act, _Slot, _value)][
                        "coref_value"
                    ]
                    if _coref_value == "":
                        continue
                ...
                if not value.isdigit():
                    for _turn in prev_turns[::-1]:
                        ...
                        for i, j, n in matches:
                            ...
                            ) > 92 or possible_value.startswith(
...
def merge_data_annotation():
    ...
    data25 = json.load(
        open(os.path.join(self_dir, extract_dir, "data_meta_fixed.json"))
    )
    ...
    if not os.path.exists("data.zip") or not os.path.exists("ontology.json"):
        ...
        for dialog_id, dialog in tqdm(data.items()):
            ...
            converted_dialogue = {
                ...
                "domains": [
                    d
                    for d in dialog["goal"]
                    if len(dialog["goal"][d]) != 0
                    and d in multiwoz_desc
                    and d not in ["police", "hospital", "bus"]
                ],
                "turns": [],
            }
            ...
            for turn_id, turn in enumerate(dialog["log"]):
                ...
                for Domain_Act in das:
                    ...
                    if Domain.lower() not in converted_dialogue[
                        "domains"
                    ] and Domain.lower() not in ["general", "booking"]:
                        continue
                ...
                if role == "system":
                    ...
                    for domain in turn_state:
                        ...
                        for subdomain in ["semi", "book"]:
                            for slot in turn_state[domain][subdomain]:
                                ...
                                if (
                                    slot == "ticket"
                                ):  # or (domain == 'train' and slot == 'people'):
                    ...
                    for domain in accum_fixed_state:
                        for slot in accum_fixed_state[domain]:
                            assert isinstance(
                                accum_fixed_state[domain][slot], str
                            ), print(accum_fixed_state[domain][slot])
                ...
                if "coreference" in turn:
                    for Domain_Act in turn["coreference"]:
                        for Slot, value, coref, coref_turn, coref_pos in turn[
                            "coreference"
                        ][Domain_Act]:
                            value = value.lower()
                            coref_dict[(Domain_Act, Slot, value)] = {
                                "turn": coref_turn,
                                "pos": coref_pos,
                                "coref_value": coref,
        ...
        json.dump(
            {k: dict(Counter(v)) for k, v in state_cat_slot_values.items()},
        ...
        new_ont = {"domains": {}, "intents": {}, "binary_dialogue_act": {}}
        ...
        for d_s in slot_to_type:
            ...
            domain_ont["slots"][s]["possible_values"] = [
                _
                for _ in domain_ont["slots"][s]["possible_values"]
                if _ not in ["dontcare", ""]
            ]
        ...
        new_ont["intents"] = {
            i: {"description": multiwoz_desc["intents"][i]} for i in intent_ont
        }
        ...
        for _update in turn["state_update"]["non-categorical"]:
            if "utt_idx" in _update:
                ...
                if turn["utt_idx"] not in changed_utt_idx_and_position:
                    if (
                        utt_idx == turn["utt_idx"]
                        and start - 1 > -1
                        and turn["utterance"][start - 1] not in [" "]
                    ):
                        ...
                        turn["utterance"] = (
                            turn["utterance"][:start] + " " + turn["utterance"][start:]
                        )
                        print(turn["utterance"])
                        _update["start"] += 1
                        _update["end"] += 1
                ...
                if utt_idx not in changed_utt_idx_and_position:
                    ...
                    value = (
                        _update["fixed_value"]
                        if "fixed_value" in _update
                        and _update["fixed_value"] != "not found"
                        else _update["value"]
                    )
                    if start >= changed_utt_idx_and_position[utt_idx]:
                        if (
                            dialog["turns"][utt_idx]["utterance"][
                                _update["start"] : _update["end"]
                            ]
                            != value
                        ):
                            assert (
                                dialog["turns"][utt_idx]["utterance"][
                                    _update["start"] + 1 : _update["end"] + 1
                                ]
                                == value
                            ), print(
                                dialog["turns"][utt_idx]["utterance"],
                        ...
                        if (
                            ...
                            assert (
                                dialog["turns"][utt_idx]["utterance"][
                                    _update["start"] : _update["end"] + 1
                                ]
                            ).replace(" ", "") == value.replace(" ", ""), print(
                                dialog["turns"][utt_idx]["utterance"],
                                dialog["turns"][utt_idx]["utterance"][
                                    _update["start"] : _update["end"] + 1
                                ],
                            ...
                            print(_update)
                            _update["end"] += 1
                            _update["fixed_value"] = turn["utterance"][
                                _update["start"] : _update["end"] + 1
                            ].strip()
                            print(_update)
        if changed:
                    ...
                    if utt_idx not in changed_utt_idx_and_position:
                        ...
                        if start >= changed_utt_idx_and_position[utt_idx]:
                            ...
                            if (
                                ...
                                _update["end"] += 1
                                _update["fixed_value"] = dialog["turns"][utt_idx][
                                    "utterance"
                                ][_update["start"] : _update["end"] + 1]
                                print(_update)
    for turn in dialog["turns"]:
        ...
        for _update in turn["state_update"]["non-categorical"]:
            if "utt_idx" in _update:
                ...
                assert dialog["turns"][utt_idx]["utterance"][start:end] == value, print(
                    json.dumps(turn, indent=4),
                    [c for c in dialog["turns"][utt_idx]["utterance"][start:end]],
                    [c for c in value],
                )
    return dialog
