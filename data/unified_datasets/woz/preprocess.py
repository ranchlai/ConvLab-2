...
cat_slot_values = {
    "area": ["east", "west", "center", "north", "south"],
    "price range": ["expensive", "moderate", "dontcare", "cheap"],
}
...
def convert_da(da, utt, all_binary):
    converted = {"binary": [], "categorical": [], "non-categorical": []}
...
def get_state_update(prev_state, cur_state, usr_da, turn_idx, dialog_idx):
    ...
    ret = {"categorical": [], "non-categorical": []}
    ...
    if not os.path.exists("data.zip") or not os.path.exists("ontology.json"):
        ...
        for split in data_splits:
            ...
            data = json.load(
                open(
                    os.path.join(
                        self_dir,
                        extract_dir,
                        "original_data/woz_{}_en.json".format(split),
                    )
                )
            )
            ...
            for dialogue in data:
                ...
                ret["original_id"] = (
                    split + str(dialogue["dialogue_idx"])
                    if split != "validate"
                    else "val" + str(dialogue["dialogue_idx"])
                )
                ...
                prev_state = {
                    "restaurant": {
                        k: "" for k in woz_desc["restaurant"] if k != "domain"
                    }
                }
        ...
        for slot in all_slot_value:
            if slot in cat_slots:
                ...
                for v in all_slot_value[slot]:
                    ...
                    if (
                        v
                        not in new_ont["domains"]["restaurant"]["slots"][slot][
                            "possible_values"
                        ]
                    ):
                        new_ont["domains"]["restaurant"]["slots"][slot][
        ...
        # read from file
        all_dialogues = read_zipped_json(
