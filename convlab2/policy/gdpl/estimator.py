...
class RewardEstimator(object):
    def __init__(self, vector, pretrain=False):
        with open(
            ...
            cfg = json.load(f)
        self.irl = AIRL(
            cfg["gamma"], cfg["hi_dim"], vector.state_dim, vector.da_dim
        ).to(device=DEVICE)
    ...
    def train_irl(self, batch, epoch):
        ...
        gen_loss /= turns
        logging.debug(
            "<<reward estimator>> epoch {}, loss_real:{}, loss_gen:{}".format(
    ...
    def test_irl(self, batch, epoch, best):
        ...
        gen_loss /= turns
        logging.debug(
            "<<reward estimator>> validation, epoch {}, loss_real:{}, loss_gen:{}".format(
                epoch, real_loss, gen_loss
            )
        )
        ...
        gen_loss /= turns
        logging.debug(
            "<<reward estimator>> test, epoch {}, loss_real:{}, loss_gen:{}".format(
                epoch, real_loss, gen_loss
            )
        )
        return best
