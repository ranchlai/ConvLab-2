...
import convlab2.policy.mdrg.multiwoz.default_policy as policy
...
class EncoderRNN(nn.Module):
    def __init__(
        ...
        self.rnn = whatCellType(
            embedding_size, hidden_size, cell_type, dropout_rate=self.dropout
        )
...
class DecoderRNN(nn.Module):
    def __init__(
        ...
        if "bi" in cell_type:  # we dont need bidirectionality in decoding
            cell_type = cell_type.strip("bi")
        self.rnn = whatCellType(
            embedding_size, hidden_size, cell_type, dropout_rate=dropout
        )
...
class Model(nn.Module):
    ...
    def build_model(self):
        self.encoder = EncoderRNN(
            len(self.input_lang_index2word),
            self.emb_size,
            self.hid_size_enc,
            self.cell_type,
            self.depth,
        ...
        if self.use_attn:
            if self.attn_type == "bahdanau":
                self.decoder = SeqAttnDecoderRNN(
                    self.emb_size,
                    self.hid_size_dec,
        ...
        if self.args.mode == "train":
            self.gen_criterion = nn.NLLLoss(
                ignore_index=3, size_average=True
            )  # logsoftmax is done in decoder part
            self.setOptimizers()
        ...
        if self.beam_search:  # wenqiang style - sequicity
            ...
            for idx in range(target_tensor.size(0)):
                ...
                while True:
                    ...
                    if (
                        n.wordid.item() == EOS_token and n.prevNode != None
                    ):  # its not empty
        ...
        for sentence in decoded_words:
            ...
            for ind in sentence:
                if self.output_index2word(
                    str(int(ind.item()))
                ) == self.output_index2word(str(EOS_token)):
                    break
