...
parser.add_argument(
    "--use_attn", type=util.str2bool, nargs="?", const=True, default=False
)
parser.add_argument("--attention_type", type=str, default="bahdanau")
parser.add_argument(
    "--use_emb", type=util.str2bool, nargs="?", const=True, default=False
)
...
parser.add_argument(
    "--no_cuda", type=util.str2bool, nargs="?", const=True, default=True
)
...
parser.add_argument(
    "--load_param", type=util.str2bool, nargs="?", const=True, default=False
)
...
def train(
    ...
    loss, loss_acts, grad = model.train(
        input_tensor,
        input_lengths,
        target_tensor,
        target_lengths,
...
if __name__ == "__main__":
    ...
    with open(
        os.path.join(os.path.dirname(__file__), "data/train_dials.json"), "r"
    ) as outfile:
        train_dials = json.load(outfile)

    # Load validation file list:
    with open(
        os.path.join(os.path.dirname(__file__), "data/val_dials.json"), "r"
    ) as outfile:
