...
def loadDialogue(model, val_file, input_tensor, target_tensor, bs_tensor, db_tensor):
    ...
    for idx, (usr, sys, bs, db) in enumerate(
        ...
        tensor = [
            model.input_word2index(word) for word in usr.strip(" ").split(" ")
        ] + [
            EOS_token
        ...
        tensor = [
            model.output_word2index(word) for word in sys.strip(" ").split(" ")
        ] + [EOS_token]
...
def load_config(basename):
    try:
        with open(
            os.path.join(os.path.curdir(__file__), basename + ".json"), "rb"
        ) as f:
            return json.load(f)
    except:
        try:
            with open(
                os.path.join(os.path.curdir(__file__), basename + ".pkl"), "rb"
            ) as f:
                return pkl.load(f)
        except:
            sys.stderr.write(
