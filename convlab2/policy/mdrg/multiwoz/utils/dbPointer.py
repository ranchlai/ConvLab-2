...
def auto_download():
    ...
    urls = {
        model_path: "https://huggingface.co/ConvLab/ConvLab-2_models/resolve/main/mdrg_model.zip",
...
for domain in domains:
    db = os.path.join(
        os.path.dirname(__file__), os.pardir, "db/{}-dbase.db".format(domain)
    )
...
def table_schema(domain):
    return [
        col[1]
        for col in dbs[domain]
        .execute("PRAGMA table_info({})".format(domain))
        .fetchall()
    ]
