...
class Simulator(Policy):
    def __init__(self):
        ...
        self.goal, self.state, self.sub_goals, self.sub_goals_state = (
            None,
            None,
            None,
            None,
        ...
        if goal[-1][0] == 1:
            ...
            goal_content = " ".join(
                [
                    x[3]
                    for x in goal
                    if not isinstance(x[3], list) and x[1] not in ["出租", "地铁"]
                ]
            )
            if "出现在id" in goal_content:
                return "不独立多领域+交通"
    ...
    def state_update(self, prev_user_da, prev_sys_da):
        ...
        if sys_nooffer:
            ...
            for i, (sub_goal_id, domain, slot, value, expressed) in enumerate(
                cur_sub_goal
            ):
            ...
            if not all_constraints_expressed:
                ...
                # current: b
                for x, y in zip(cur_sub_goal, cur_sub_goal_state):
                    if (not x[3] or "id" in x[3]) and (x[1], x[2]) in [
                        (z[0], z[1]) for z in sys_inform
                    ]:
                    ...
                    if t[3]:
                        ...
                        if isinstance(t[3], str):
                            if "id" in t[3]:
                                ...
                                normal_constraints.append(
                                    [t, ["Inform", t[1], t[2], t[3]]]
                                )
                        else:
                            for v in t[3]:
                                normal_constraints.append(
