...
class RuleBasedMultiwozBot(Policy):
    ...
    def predict(self, state):
        ...
        if (
            "user_action" in state
            and (len(state["user_action"]) > 0)
            and type(state["user_action"]) is not str
        ):
            user_action = {}
        ...
        if (
            len(
                [
                    domain_intent
                    for domain_intent, slots in DA.items()
                    if slots or "nooffer" in domain_intent.lower()
                ]
            )
            == 0
        ):
    ...
    def _update_DA(self, user_act, user_action, state, DA):
        ...
        if intent_type == "Request":
            ...
            if len(kb_result) == 0:
                ...
                for slot in state["belief_state"][domain.lower()]["semi"]:
                    if state["belief_state"][domain.lower()]["semi"][
                        slot
                    ] != "" and state["belief_state"][domain.lower()]["semi"][
                        slot
                    ] not in [
                        "do nt care",
                        "do n't care",
