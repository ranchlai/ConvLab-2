...
class SysPerfectBD2Word(BaseModel):
    ...
    def forward(
        ...
        dec_init_state = self.policy(
            th.cat([bs_label, db_label, utt_summary.squeeze(1)], dim=1)
        ).unsqueeze(0)
    ...
    def forward_rl(self, data_feed, max_words, temp=0.1):
        ...
        # decode
        logprobs, outs = self.decoder.forward_rl(
            batch_size=batch_size,
...
class SysPerfectBD2Cat(BaseModel):
    def __init__(self, corpus, config):
        ...
        self.c2z = nn_lib.Hidden2Discrete(
            self.utt_encoder.output_size + self.db_size + self.bs_size,
            config.y_size,
            config.k_size,
            is_lstm=False,
        ...
        self.z_embedding = nn.Linear(
            self.y_size * self.k_size, config.dec_cell_size, bias=False
        )
...
class SysPerfectBD2Gauss(BaseModel):
    def __init__(self, corpus, config):
        ...
        self.c2z = nn_lib.Hidden2Gaussian(
            self.utt_encoder.output_size + self.db_size + self.bs_size,
            config.y_size,
            is_lstm=False,
        )
