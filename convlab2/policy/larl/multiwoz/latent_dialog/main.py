...
class Reinforce(object):
    def __init__(
        ...
        if self.rl_config.record_freq > 0:
            self.ppl_exp_file = open(
                os.path.join(self.rl_config.record_path, "ppl.tsv"), "w"
            )
            self.rl_exp_file = open(
                os.path.join(self.rl_config.record_path, "rl.tsv"), "w"
            )
            self.learning_exp_file = open(
                os.path.join(self.rl_config.record_path, "learning.tsv"), "w"
            )
    ...
    def run(self):
        ...
        for ctxs in self.ctx_gen.iter(self.rl_config.nepoch):
            ...
            # supervised learning
            if (
                self.rl_config.sv_train_freq > 0
            ...
            # roll out and learn
            _, agree, rl_reward, rl_stats = self.dialog.run(
                ctxs, verbose=n % self.rl_config.record_freq == 0
            )
            ...
            if self.rl_config.record_freq > 0 and n % self.rl_config.record_freq == 0:
                ...
                if aver_reward > best_rl_reward:
                    print(
                        "[INFO] Update on reward in Epsd {} ({} > {})".format(
                            n, aver_reward, best_rl_reward
                        )
                    )
                    th.save(
                        self.sys_model.state_dict(),
                        self.rl_config.reward_best_model_path,
                    )
                    best_rl_reward = aver_reward
                else:
                    print(
                        "[INFO] No update on reward in Epsd {} ({} < {})".format(
                            n, aver_reward, best_rl_reward
                        )
                    )
            ...
            for epoch_id in range(self.rl_config.nepoch):
                ...
                while True:
                    ...
                    if (
                        ...
                        self.train_func(
                            self.sys_model, self.sl_train_data, self.sv_config
                        )
                    ...
                    if (
                        ...
                        self.learning_exp_file.write(
                            "{}\t{}\n".format(
                                n,
                                np.mean(
                                    self.agent.all_rewards[
                                        -self.rl_config.record_freq :
                                    ]
                                ),
                            )
                        )
...
def train(model, train_data, val_data, test_data, config, evaluator, gen=None):
    ...
    while True:
        train_data.epoch_init(
            config,
            shuffle=True,
            verbose=done_epoch == 0,
            fix_batch=config.fix_train_batch,
        )
        while True:
            batch = train_data.next_batch()
            if batch is None:
            ...
            if batch_cnt % config.ckpt_step == 0:
                ...
                if valid_loss < best_valid_loss:
                    ...
                    if config.save_model:
                        ...
                        logger.info(
                            "!!Model Saved with loss = {},at {}.".format(
                                valid_loss, cur_time
                            )
                        )
                        th.save(
...
def generate(model, data, config, evaluator, num_batch, dest_f=None):
    ...
    while True:
        ...
        if config.dec_use_attn:
            ...
            )  # (batch_size, max_dec_len, max_ctx_len)
        else:
            pred_attns = None
        # get context
