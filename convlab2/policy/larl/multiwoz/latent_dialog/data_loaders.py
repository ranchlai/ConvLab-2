...
class DealDataLoaders(BaseDataLoaders):
    ...
    def flatten_dialog(self, data, backward_size):
        ...
        for dlg in data:
            ...
            for i in range(1, len(dlg.dlg)):
                ...
                response["utt"] = self.pad_to(
                    self.max_utt_len, response.utt, do_pad=False
                )
                context = []
    ...
    def epoch_init(self, config, shuffle=True, verbose=True, fix_batch=False):
        super(DealDataLoaders, self).epoch_init(
            config, shuffle=shuffle, verbose=verbose
        )
...
class BeliefDbDataLoaders(BaseDataLoaders):
    def __init__(self, name, data, config):
        ...
        self.max_utt_len = config.max_utt_len
        self.data, self.indexes, self.batch_indexes = self.flatten_dialog(
            data, config.backward_size
        )
        self.data_size = len(self.data)
        self.domains = [
            "hotel",
    ...
    def epoch_init(self, config, shuffle=True, verbose=True, fix_batch=False):
        ...
        if fix_batch:
            ...
            if verbose:
                print(
                    "Number of left over sample = %d"
                    % (self.data_size - config.batch_size * self.num_batch)
                )
        if shuffle:
            if fix_batch:
    ...
    def _prepare_batch(self, selected_index):
        ...
        max_ctx_len = np.max(vec_ctx_lens)
        vec_ctx_utts = np.zeros(
            (batch_size, max_ctx_len, self.max_utt_len), dtype=np.int32
        )
