        ...
        for index, token in enumerate(tokens):
            if token.startswith("[") and (
                ...
                if domain == "train" and slot == "id":
                    ...
                    slot = "Ref"
                if (
                    domain in top_results
                    and len(top_results[domain]) > 0
                    and slot in top_results[domain]
                ):
                    ...
                    if slot == "count":
                        ...
                        if (
                            "arrive" in " ".join(response[-3:])
                            or "arrival" in " ".join(response[-3:])
                            or "arriving" in " ".join(response[-3:])
                        ):
                            if (
                            ...
                            or "leaving" in " ".join(response[-3:])
                            or "departure" in " ".join(response[-3:])
                        ):
                            if (
                                active_domain is not None
    ...
    def model_predict(self, data_feed):
        ...
        if self.model.config.dec_use_attn:
            ...
            temp_sample_y = sample_y.view(
                -1, self.model.config.y_size, self.model.config.k_size
            )
        ...
        dec_outputs, dec_hidden_state, ret_dict = self.model.decoder(
            ...
            mode=mode,
            gen_type="greedy",
            beam_size=self.model.config.beam_size,
...
if __name__ == "__main__":
    ...
    train_data_path = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "data/train_dials.json"
    )
