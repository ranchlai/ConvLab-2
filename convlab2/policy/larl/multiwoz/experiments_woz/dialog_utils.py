...
def task_generate(
    ...
    task_report, success, match = evaluator.evaluateModel(
        generated_dialogs, mode=data.name
    )
...
def dump_latent(model, data, config):
    ...
    while True:
        ...
        sample_y = (
            ...
            .cpu()
            .data.numpy()
            .reshape(-1, config.y_size, config.k_size)
        )
        log_qy = (
            outputs["log_qy"]
            .cpu()
        ...
        for b_id in range(pred_labels.shape[0]):
            ...
            latent_results[keys[b_id]].append(
                {
                    "context": prev_ctx,
                    "gt_resp": true_str,
                    "pred_resp": pred_str,
                    "domain": batch["goals_list"],
                    "sample_y": sample_y[b_id],
                    "log_qy": log_qy[b_id],
