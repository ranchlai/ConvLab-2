...
def parse_opt():
    ...
    parser.add_argument(
        "--emb_dim", type=int, default=128, help="the embedding dimension"
    )
    parser.add_argument("--epoch", type=int, default=360, help="train epoches")
    parser.add_argument(
        "--dropout", type=float, default=0.2, help="the embedding dimension"
    )
    parser.add_argument(
        "--resume",
        action="store_true",
        default=False,
        help="whether to resume previous run",
    )
    parser.add_argument(
        "--batch_size", type=int, default=256, help="the embedding dimension"
    )
    parser.add_argument(
        "--model", type=str, default="BERT", help="the embedding dimension"
    )
    parser.add_argument(
        "--data_dir", type=str, default="data", help="the embedding dimension"
    )
    parser.add_argument(
...
if "train" in args.option:
    *train_examples, _ = get_batch(
        args.data_dir, "train", tokenizer, args.max_seq_length
    )
    ...
    *val_examples, val_id = get_batch(
        args.data_dir, "val", tokenizer, args.max_seq_length
    )
    ...
    *val_examples, val_id = get_batch(
        args.data_dir, "test", tokenizer, args.max_seq_length
    )
...
if "BERT" in args.model:
    if args.field:
        ...
        decoder = TransformerDecoder(
            vocab_size=tokenizer.vocab_len,
            d_word_vec=args.emb_dim,
            act_dim=len(Constants.act_ontology),
            n_layers=args.layer_num,
            d_model=args.emb_dim,
            n_head=args.head,
            dropout=args.dropout,
...
if args.option == "train":
    ...
    for batch_step, batch in enumerate(eval_dataloader):
        ...
        logger.info(
            "finished {}/{} used {} sec/per-sent".format(
                batch_step,
                len(eval_dataloader),
