    ...
    for num_dial, dialog_info in enumerate(source):
        ...
        for turn_num, turn in enumerate(dialog):
            ...
            if predicted_acts is not None:
                hierarchical_act_vecs = np.asarray(
                    predicted_acts[dialog_file][str(turn_num)], "int64"
                )
            else:
