...
class Sclstm(nn.Module):
    ...
    def _step(self, input_t, last_hidden, last_cell, last_dt, src_enc, src_mask):
        ...
        * Do feedforward for one step *
        ...
        # updata dt
        alpha = 0.5
        gate_r = torch.sigmoid(
            torch.relu(self.w2h_r(input_t) + alpha * self.h2h_r(last_hidden))
        )
        dt = gate_r * last_dt
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.tgt_word_emb = nn.Embedding(
            vocab_size, d_word_vec, padding_idx=Constants.PAD
        )
    ...
    def forward(self, tgt_seq, src_seq, act_vecs):
        ...
        # -- Forward
        dec_output = (
            self.tgt_word_emb(tgt_seq)
            + self.post_word_emb(tgt_seq)
            + self.act_word_emb(act_vecs)[:, None, :]
        )
    ...
    def translate_batch(self, act_vecs, src_seq, n_bm, max_token_seq_len=30):
        ...
        def collate_active_info(
            ...
            active_act_vecs = collect_active_part(
                act_vecs, active_inst_idx, n_prev_active_inst, n_bm
            )
        ...
        def beam_decode_step(
            ...
            dec_partial_seq = [
                inst_dec_beams[idx].get_current_state()
        ...
        def collect_hypothesis_and_scores(inst_dec_beams, n_best):
            ...
            for beam in inst_dec_beams:
                scores = beam.scores
                hyps = np.array(
                    [beam.get_hypothesis(i) for i in range(beam.size)], "long"
                )
                lengths = (hyps != Constants.PAD).sum(-1)
                normed_scores = [
                    scores[i].item() / lengths[i] for i, hyp in enumerate(hyps)
                ]
