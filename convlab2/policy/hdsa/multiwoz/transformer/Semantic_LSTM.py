...
class SCLSTM(nn.Module):
    def __init__(
        self, vocab_size, d_word_vec, d_model, act_size, n_layer=1, dropout=0.5
    ):
    ...
    def rnn_step(self, last_inp, last_hidden, last_cell, last_dt, gen=False):
            ...
            if gen:
                ...
                output_hidden.append(
                    nn.functional.dropout(
                        _hidden.clone(), p=self.dropout, training=True
                    )
                )
        ...
        if not gen:
            for i in range(self.n_layer):
                last_hidden[i] = nn.functional.dropout(
        ...
        **kwargs
        ...
        for t in range(max_len):
            output, last_hidden, last_cell, last_dt = self.rnn_step(
                last_inp, last_hidden, last_cell, last_dt, gen=gen
            )
    ...
    def translate_batch(
        self, act_vecs, src_enc, gen=True, n_bm=10, max_token_seq_len=50
    ):
        """
        Args:
        ...
        if n_bm > 1:  # wenqiang style - sequicity
            ...
            for idx in range(batch_size):
                ...
                node = BeamSearchNode(
                    (decoder_hidden, decoder_cell, decoder_act),
                    None,
                    decoder_input,
                    0,
                    1,
                ...
                while True:
                    ...
                    if (
                        n.wordid.item() == Constants.EOS and n.prevNode != None
                    ):  # its not empty
