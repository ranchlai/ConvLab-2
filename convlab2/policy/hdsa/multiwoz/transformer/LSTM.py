...
class LSTMDecoder(nn.Module):
    ...
    def forward(self, tgt_seq, enc_output, act_vecs):
        ...
        proba = torch.zeros(
            batch_size, target_length, self.vocab_size, device=tgt_seq.device
        )  # [B,T,V]
        for t in range(target_length):
            decoder_output, decoder_hidden = self.cell(
                decoder_input, decoder_hidden, None
            )

            use_teacher_forcing = (
                True if random.random() < self.teacher_ratio else False
            )
            if use_teacher_forcing:
                if t + 1 < target_length:
                    decoder_input = tgt_seq[:, t + 1].view(
                        -1, 1
                    )  # [B,1] Teacher forcing
            else:
    ...
    def translate_batch(
        self, act_vecs, src_enc, n_bm, max_token_seq_len=30, return_all=False
    ):
