...
ref_slot_data2stand = {
    "train": {"duration": "time", "price": "ticket", "trainid": "id"}
}
...
class UserDataManager(object):
    ...
    def usrgoal2seq(goal: dict):
        ...
        ret = []
        for (domain, domain_goal) in filter(
            lambda x: (x[0] in domains_set and len(x[1]) > 0),
            ...
            if goal_slot is not None and goal_slot != "Unknow":
                ...
                if True in check.values() or False in check.values():
                    ...
                    if True in check.values() and False in check.values():
                        ...
                        True in check.values() and False not in check.values()
                    ):
                        ret += "[R]"
                    elif (
                        True not in check.values() and False in check.values()
                    ):
                        ret += "[W]"
    ...
    def usrseq2da(usr_seq: list, goal: dict):
        ...
        for word in usr_seq:
            ...
            # slot-value
                ...
                slot_goal = UsrDa2Goal.get(domain.lower(), {}).get(
                    slot_da, None
                )
        ...
        for act in da.keys():
            if "-" in act:
                ...
                for idx in range(len(da[act])):
                    da[act][idx][0] = ref_slot_data2stand.get(
                        domain.lower(), {}
                    ).get(da[act][idx][0], da[act][idx][0])
    ...
    def org_data_loader(self):
        if (
            self.__org_goals is None
            or self.__org_usr_dass is None
            or self.__org_sys_dass is None
        ):
            ...
            for session in full_data.values():
                ...
                for turn in range(len(logs) // 2):
                    ...
                    usr_da = self.ref_data2stand(
                        logs[turn * 2].get("dialog_act", {})
                    )
                    sys_da = self.ref_data2stand(
                        logs[turn * 2 + 1].get("dialog_act", {})
                    )
    ...
    def vocab_loader(self):
        if (
            self.__voc_goal is None
            or self.__voc_usr is None
            or self.__voc_usr_rev is None
            or self.__voc_sys is None
        ):
    ...
    def train_test_val_split(
        ...
        return (
            np.array(goals)[idx_train],
            np.array(usr_dass)[idx_train],
            np.array(sys_dass)[idx_train],
            np.array(goals)[idx_test],
            np.array(usr_dass)[idx_test],
            np.array(sys_dass)[idx_test],
            np.array(goals)[idx_val],
            np.array(usr_dass)[idx_val],
            np.array(sys_dass)[idx_val],
        )

    def data_loader_seg(self):
        if (
            self.__goals_seg is None
            or self.__usr_dass_seg is None
    ...
    @staticmethod
    def train_test_val_split_seg(
        goals_seg, usr_dass_seg, sys_dass_seg, test_size=0.1, val_size=0.1
    ):
        def dr(dss):
        ...
        idx_train = random.sample(idx_train, len(idx_train))
        return (
            dr(np.array(goals_seg)[idx_train]),
            dr(np.array(usr_dass_seg)[idx_train]),
            dr(np.array(sys_dass_seg)[idx_train]),
            dr(np.array(goals_seg)[idx_test]),
            dr(np.array(usr_dass_seg)[idx_test]),
            dr(np.array(sys_dass_seg)[idx_test]),
            dr(np.array(goals_seg)[idx_val]),
            dr(np.array(usr_dass_seg)[idx_val]),
            dr(np.array(sys_dass_seg)[idx_val]),
        )
    ...
    def get_sysda_id(self, sys_das):
        return [
            [self.voc_sys.get(word, self.voc_sys[self.unk]) for word in sys_da]
            for sys_da in sys_das
        ]
