...
class VHUS(nn.Module):
    def __init__(self, cfg, voc_goal_size, voc_usr_size, voc_sys_size):
        ...
        self.terminated_net = nn.Sequential(
            nn.Linear(cfg["hu_dim"], cfg["hu_dim"]),
            nn.ReLU(),
            nn.Linear(cfg["hu_dim"], 1),
        )
        self.usr_decoder = Decoder(
    ...
    def forward(
        ...
        a_weights, _, _ = self.usr_decoder(
            inputs=origin_responses,
            encoder_hidden=hidden.unsqueeze(0),
    ...
    def select_action(self, goal, goal_length, post, post_length):
        ...
        goal, goal_length, post, post_length = (
            goal.to(device=DEVICE).unsqueeze(0),
            goal_length.to(device=DEVICE).unsqueeze(0),
            post.to(device=DEVICE).unsqueeze(0),
    ...
    def forward(
        ...
        def decode(step, step_output, step_attn, infer=False):
            ...
            if infer and not step:
                symbols = torch.cat(
                    (
                        decoder_outputs[-1][:, : self.eos_id],
                        decoder_outputs[-1][:, (self.eos_id + 1) :],
                    ),
                    1,
                ).topk(1)[1]
