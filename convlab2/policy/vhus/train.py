...
class VHUS_Trainer:
    def __init__(self, config, manager, goal_gen):
        ...
        self.user = VHUS(config, voc_goal_size, voc_usr_size, voc_sys_size).to(
            device=DEVICE
        )
    ...
    def user_loop(self, data):
        ...
        a_weights, t_weights, argu = self.user(
            batch_input["goals"],
            batch_input["goals_length"],
            batch_input["posts"],
            batch_input["posts_length"],
    ...
    def imitating(self, epoch):
        ...
        for i, data in enumerate(data_train_iter):
            ...
            if (i + 1) % self.print_per_batch == 0:
                ...
                logging.debug(
                    "<<user simulator>> epoch {}, iter {}, loss_a:{}, loss_t:{}".format(
                        epoch, i, a_loss, t_loss
                    )
                )
                a_loss, t_loss = 0.0, 0.0
    ...
    def imit_test(self, epoch, best):
        ...
        t_loss /= i + 1
        logging.debug(
            "<<user simulator>> validation, epoch {}, loss_a:{}, loss_t:{}".format(
                epoch, a_loss, t_loss
            )
        )
        loss = a_loss + t_loss
