...
class MultiWozVector(Vector):
    ...
    def generate_dict(self):
        ...
        for domain in self.belief_domains:
            for slot, value in default_state()["belief_state"][domain.lower()][
                "semi"
            ].items():
    ...
    def state_vectorize(self, state):
        ...
        for da in action:
            if da in self.act2vec:
                last_act_vec[self.act2vec[da]] = 1.0
    ...
    def action_devectorize(self, action_vec):
        ...
        action = lexicalize_da(
            action, entities, self.state, self.requestable, self.cur_domain
        )
        return action
        ...
        if self.composite_actions:
            ...
            for act in self.act2vec:
                if set(action) == set(act.split(";")):
                    act_vec[self.act2vec[act]] = 1.0
                    break
        else:
            for da in action:
                if da in self.act2vec:
                    act_vec[self.act2vec[da]] = 1.0
        return act_vec
