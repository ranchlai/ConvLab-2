...
class MultiSourceDBLoader(BaseDBLoader):
    ...
    def __init__(
        self,
        args: Union[
            List[MultiSourceDBLoaderArgs], List[tuple], MultiSourceDBLoaderArgs
        ],
    ):
    ...
    def load_db(
        ...
        for loader, domain_slot_map in self.loaders_and_maps:
            if slot is not None:
                ...
                dbs.extend(
                    (db, db_domain, domain_slot_map)
                    for db_domain, db in domain_to_db.items()
                )
...
class MultiSourceDB(BaseDB):
    ...
    def sample_value(self, domain, slot, excluding_values=()):
        values = self.find_different_values(
            domain, slot, excluding_values=excluding_values
        )
        try:
...
def _get_positions(
    words: List[str], word_to_indexes: Dict[str, List[int]], value: List[str]
):
    first_word = value[0]
    N = len(value)
...
def assert_correct_turn(turn: dict):
    ...
    for item in span_info:
        ...
        assert words[begin : 1 + end] == tokenize(
            value
        ), f"turn: {p_str(turn)}\nitem: {item}"
...
def replace_slot_values(
    ...
    for turn_index, turn in enumerate(sample["log"]):
        ...
        new_turn = replace_slot_values_in_turn(
            turn, db_loader, p=p, inform_intents=inform_intents
        )
