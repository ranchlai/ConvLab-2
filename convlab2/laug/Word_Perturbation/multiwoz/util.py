...
def _get_all_slots(multiwoz: MultiwozDatasetType):
    slots = defaultdict(
        dict
    )  # Dict[str, Dict[str, Set[str]]]; Dict[Sample_ID, Dict[intent, Set[Slot]]]
        ...
        for turn in logs:
            ...
            for domain_intent, slot_value_list in dialog_act.items():
                ...
                for slot, value in slot_value_list:
                    slots[domain].setdefault(intent, set()).add(
                        slot.lower() if isinstance(slot, str) else slot
                    )
    for domain in slots:
...
class Helper:
    ...
    _words_about_slot = {
        ...
        "hotel": {
            ...
            "stars": "stars",
            "name": "name",
            "stay": ["stay", Patterns.integer_pattern],
            "day": "day",
        ...
        "taxi": {
            "leave": ["leaveat", "leave at", Patterns.time_pattern],
            "dest": ["destination", "cambridge", "belfry"],
            "depart": "departure",
    ...
    def relevant_words_of_slot(self, domain, slot):
        if (
            domain not in self._words_about_slot
            or slot not in self._words_about_slot[domain]
        ):
            return [slot]
            ...
            for word in self.relevant_words_of_domain(domain):
                if isinstance(word, str):
                    ...
                    if len(ws) == 1:
                        ...
                        for i in range(N):
                            if i + n <= N and all(
                                ws[j] == words[i + j] for j in range(n)
                            ):
...
def _equal_words(words1, words2, ignore_case):
    if not ignore_case:
        ...
        return len(words1) == len(words2) and all(
            w1.lower() == w2.lower() for w1, w2 in zip(words1, words2)
        )


def is_span_info_consistent_with_text(
    sentence: SentenceType, span_info: List[list], ignore_case=True
) -> bool:
    """check whether the span info is consistent with text."""
    words = convert_sentence_to_tokens(sentence)
    return all(
        _equal_words(words[start : end + 1], tokenize(span), ignore_case)
        for domain_intent, slot, span, start, end in span_info
    ) and len({tuple(x[-2:]) for x in span_info}) == len(span_info)
