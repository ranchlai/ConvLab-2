# -*- coding: utf-8 -*-
from convlab2.laug.Word_Perturbation.frames.aug_with_sgd_db import (
    frames_eda_config,
)
from convlab2.laug.Word_Perturbation.multiwoz.aug_with_sgd_db import (
    multiwoz_eda_config,
)
...
class Word_Perturbation:
    def __init__(self, dataset="multiwoz"):
        ...
        if dataset == "multiwoz":
            ...
            self.EDA = MultiwozEDA(
                multiwoz_config.multiwoz, multiwoz_config.db_loader
            )
        elif dataset == "frames":
            frames_config = frames_eda_config()
            self.EDA = MultiwozEDA(
                frames_config.frames, frames_config.db_loader
            )

    def aug(self, text, span_info):
        (new_text, new_span_info, _), _ = self.EDA.augment_sentence_only(
            text, span_info, {}
        )
