    ...
    if str(number).isnumeric():
        ...
        if int(number) < 20:
            ...
            if str(number)[1] == "0":
                ...
                return (
                    IN_HUNDRED_CONSTANT[int(str(number)[0])]
                    + " "
                    + NUMBER_CONSTANT[int(str(number)[1])]
                )
        else:
            ...
            for groupNumber in numberArray:
                if groupCount > 1 and groupNumber[0:] != "000":
                    stringResult += (
                        str(getUnderThreeNumberString(str(groupNumber))) + " "
                    )
                else:
                    break
                groupCount -= 1
...
def getUnderThreeNumberString(number):
    if str(number).isnumeric() and len(number) < 4:
        if len(number) < 3:
            ...
            return (
                NUMBER_CONSTANT[int(number[0])]
                + "  "
                + BASE_CONSTANT[1]
                + " and "
                + translateNumberToEnglish((number[1:]))
            )
