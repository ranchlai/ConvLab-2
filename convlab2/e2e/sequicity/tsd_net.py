...
def get_sparse_input_aug(x_input_np):
    ...
    result = np.zeros(
        (
            x_input_np.shape[0],
            x_input_np.shape[1],
            cfg.vocab_size + x_input_np.shape[0],
        ),
        dtype=np.float32,
    )
    result.fill(1e-10)
...
class BSpanDecoder(nn.Module):
    ...
    def forward(
        ...
        if pv_z_enc_out is not None:
            context = self.attn_u(
                last_hidden, torch.cat([pv_z_enc_out, u_enc_out], dim=0)
            )
        else:
        ...
        if pv_z_enc_out is None:
            ...
            sparse_pv_z_input = Variable(
                get_sparse_input_aug(prev_z_input_np), requires_grad=False
            )
            pv_z_copy_score = F.tanh(
                self.proj_copy2(pv_z_enc_out.transpose(0, 1))
            )  # [B,T,H]
            ...
            proba = (
                ...
                + pv_z_copy_score[:, : cfg.vocab_size]
            )  # [B,V]
            proba = torch.cat(
                [
                    proba,
...
class ResponseDecoder(nn.Module):
    ...
    def forward(
        ...
        sparse_z_input = Variable(
            self.get_sparse_selective_input(z_input_np), requires_grad=False
        )
...
class TSD(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.emb = nn.Embedding(vocab_size, embed_size)
        self.dec_gru = nn.GRU(
            degree_size + embed_size + hidden_size * 2,
            hidden_size,
    ...
    def forward_turn(
        ...
        **kwargs
        ...
        if mode == "train":
            ...
            pz_proba = []
            z_length = (
                z_input.size(0) if z_input is not None else self.z_length
            )  # GO token
            ...
            if mode == "test":
                ...
                if not self.beam_search:
                    ...
                    m_output_index = self.beam_search_decode(
                        pz_dec_outs,
                        u_enc_out,
                        m_tm1,
                        u_input_np,
            ...
            while k < len(states) and k < self.beam_size - dead_k:
                ...
                for new_k in range(self.beam_size - dead_k):
                    ...
                    if self.vocab.decode(decoded_t.data[0]) == cfg.eos_m_token:
                        ...
                        new_state = state.update_clone(
                            score_incre, last_hidden, decoded_t
                        )
    ...
    def beam_search_decode(
        ...
        vars = (
            torch.split(pz_dec_outs, 1, dim=1),
            torch.split(u_enc_out, 1, dim=1),
            torch.split(m_tm1, 1, dim=1),
            torch.split(last_hidden, 1, dim=1),
            torch.split(degree_input, 1, dim=0),
        )
        decoded = []
        for i, (
            pz_dec_out_s,
            ...
            decoded_s = self.beam_search_decode_single(
                pz_dec_out_s,
                u_enc_out_s,
                m_tm1_s,
    ...
    def sampling_decode(
        ...
        for i, (
            ...
            for j in range(sample_num):
                loss = self.sampling_decode_single(
                    pz_dec_out_s,
                    u_enc_out_s,
                    m_tm1_s,
                    u_input_np[:, i].reshape((-1, 1)),
