...
class CamRestEvaluator(GenericEvaluator):
    ...
    def _extract_request(self, z):
        z = z.split()
        return set(z).intersection(
            ["address", "postcode", "phone", "area", "pricerange", "food"]
        )
...
class KvretEvaluator(GenericEvaluator):
    ...
    def run_metrics(self):
        data = self.read_result_data()
        for i, row in enumerate(data):
            data[i]["response"] = self.clean_by_intent(
                data[i]["response"], int(data[i]["dial_id"])
            )
            data[i]["generated_response"] = self.clean_by_intent(
                data[i]["generated_response"], int(data[i]["dial_id"])
            )
    ...
    @report
    def match_rate_metric(
        self, data, sub="match", bspans="./data/kvret/test.bspan.pkl"
    ):
        dials = self.pack_dial(data)
        match, total = 0, 1e-8
