...
from convlab2.e2e.sequicity.metric import (
    CamRestEvaluator,
    KvretEvaluator,
    MultiWozEvaluator,
)
...
class Model:
    ...
    def _convert_batch(self, py_batch, prev_z_py=None):
        ...
        if cfg.prev_z_method == "concat" and prev_z_py is not None:
            for i in range(len(u_input_py)):
                ...
                if (
                    eob in prev_z_py[i]
                    and prev_z_py[i].index(eob) != len(prev_z_py[i]) - 1
                ):
    ...
    def train(self):
        ...
        for epoch in range(cfg.epoch_num):
            ...
            optim = Adam(
                lr=lr,
                params=filter(lambda x: x.requires_grad, self.m.parameters()),
                weight_decay=1e-5,
            )
            for iter_num, dial_batch in enumerate(data_iterator):
                ...
                for turn_num, turn_batch in enumerate(dial_batch):
                    ...
                    loss, pr_loss, m_loss, turn_states = self.m(
                        u_input=u_input,
                        z_input=z_input,
                        m_input=m_input,
                    ...
                    sup_cnt += 1
                    logging.debug(
                        "loss:{} pr_loss:{} m_loss:{} grad:{}".format(
                            loss.item(), pr_loss.item(), m_loss.item(), grad
                        )
                    )
            ...
            logging.info(
                "validation loss in epoch %d sup:%f unsup:%f"
    ...
    def eval(self, data="test"):
        ...
        for batch_num, dial_batch in enumerate(data_iterator):
            ...
            for turn_num, turn_batch in enumerate(dial_batch):
                ...
                m_idx, z_idx, turn_states = self.m(
                    mode=mode,
                    u_input=u_input,
                    u_len=u_len,
                    z_input=z_input,
    ...
    def validate(self, data="dev"):
        ...
        for dial_batch in data_iterator:
            ...
            for turn_num, turn_batch in enumerate(dial_batch):
                ...
                sup_cnt += 1
                logging.debug(
                    "loss:{} pr_loss:{} m_loss:{}".format(
                        loss.item(), pr_loss.item(), m_loss.item()
                    )
                )
    ...
    def load_glove_embedding(self, freeze=False):
        ...
        embedding_arr = torch.from_numpy(
