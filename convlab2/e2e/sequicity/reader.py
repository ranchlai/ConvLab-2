...
class _ReaderBase:
            ...
            if len(l) + len(self._idx2item) < limit:
                logging.warning(
                    "actual label set smaller than that configured: {}/{}".format(
                        len(l) + len(self._idx2item), limit
                    )
                )
            for item in l:
            ...
            for dial in batch:
                for turn in dial:
                    ...
                    if not turn["supervised"]:
                        turn["degree"] = [
                            0.0
                        ] * cfg.degree_size  # unsupervised learning. DB degree should be unknown
        return all_batches
        ...
        for i in range(batch_size):
            ...
            for key in turn_batch:
                ...
                if key in eos_syntax:
                    entry[key] = self.vocab.sentence_decode(
                        entry[key], eos=eos_syntax[key]
                    )
            if gen_m:
                entry["generated_response"] = self.vocab.sentence_decode(
                    gen_m[i], eos="EOS_M"
                )
            else:
                entry["generated_response"] = ""
            if gen_z:
                entry["generated_bspan"] = self.vocab.sentence_decode(
                    gen_z[i], eos="EOS_Z2"
                )
            else:
    ...
    def _degree_vec_mapping(self, match_num):
        l = [0.0] * cfg.degree_size
        l[min(cfg.degree_size - 1, match_num)] = 1.0
        return l
...
class CamRest676Reader(_ReaderBase):
        ...
        for v, k in sorted(vk_map.items(), key=lambda x: -len(x[0])):
            ...
            if k not in ["name", "address"]:
                response = clean_replace(
                    response, v, k + "_SLOT", forward=True, backward=False
                )
            else:
                response = clean_replace(
                    response, v, k + "_SLOT", forward=False, backward=False
                )
        return response
    ...
    def _split_data(self, encoded_data, split):
        ...
        train, dev, test = (
            encoded_data[:dev_thr],
            encoded_data[dev_thr:test_thr],
            encoded_data[test_thr:],
        )
...
class KvretReader(_ReaderBase):
    ...
    def _construct(
        self, train_json_path, dev_json_path, test_json_path, entity_json_path
    ):
        construct_vocab = False
        if not os.path.isfile(cfg.vocab_path):
            construct_vocab = True
        ...
        train_json, dev_json, test_json = (
            open(train_json_path),
            open(dev_json_path),
            open(test_json_path),
        )
        entity_json = open(entity_json_path)
        train_data, dev_data, test_data = (
            json.loads(train_json.read().lower()),
            json.loads(dev_json.read().lower()),
    ...
    def _replace_entity(self, response, vk_map, prev_user_input, intent):
        ...
        response = re.sub(
            "\d+\s\w+\s(dr)?(ct)?(rd)?(road)?(st)?(ave)?(way)?(pl)?\w*[.]?",
            "address_SLOT",
            response,
        )
        ...
        for dial_id, raw_dial in enumerate(raw_data):
                ...
                if dial_turn["turn"] == "driver":
                    ...
                    if not constraint_dict:
                        ...
                        for k, v in dial_turn["data"]["slots"].items():
                            constraint_dict[k] = v
                    constraint_dict = self._clean_constraint_dict(
                        constraint_dict, intent
                    )
                    ...
                    dataset_requested = set(
                        filter(
                            lambda x: dial_turn["data"]["requested"][x],
                            dial_turn["data"]["requested"].keys(),
                        )
                    )
                    requestable = {
                        "weather": ["weather_attribute"],
                    ...
                    single_turn["degree"] = self.db_degree(
                        constraints, raw_dial["scenario"]["kb"]["items"]
                    )
            ...
            if add_to_vocab:
                for single_turn in tokenized_dial:
                    for word_token in (
                        single_turn["constraint"]
                        + single_turn["requested"]
                        + single_turn["user"]
...
class MultiWozReader(_ReaderBase):
    ...
    def _construct(
        ...
        train_tokenized_data = self._get_tokenized_data(
            train_raw_data, db_data, construct_vocab
        )
        dev_tokenized_data = self._get_tokenized_data(
            dev_raw_data, db_data, construct_vocab
        )
        test_tokenized_data = self._get_tokenized_data(
            test_raw_data, db_data, construct_vocab
        )
        if construct_vocab:
            self.vocab.construct(cfg.vocab_size)
            self.vocab.save_vocab(cfg.vocab_path)
    ...
    def wrap_result(
        ...
        for i in range(batch_size):
            ...
            if gen_m:
                ...
                constraints = (
                    constraint_request[: constraint_request.index("EOS_Z1")]
                    if "EOS_Z1" in constraint_request
                    else constraint_request
                )
