...
class MultiWozReader(object):
    def __init__(self):
        ...
        self.delex_sg_valdict_path = os.path.join(
            default_path, "delex_single_valdict.json"
        )
    ...
    def prepare_input_np(self, u, u_delex):
        ...
        py_batch = {}
        py_batch["user"] = [
            self.vocab.sentence_encode(u.split() + [eos_tokens["user"]])
        ]
        ...
        if self.first_turn:
            ...
            for item, py_list in self.py_prev.items():
                ...
                if item in ["pv_resp", "pv_bspn"]:
                    ...
                    inputs[item + "_unk_np"][
                        inputs[item + "_unk_np"] >= self.vocab_size
                    ] = 2  # <unk>
                else:
    ...
    def bspan_to_constraint_dict(self, bspan, bspn_mode="bspn"):
        ...
        for idx, cons in enumerate(bspan):
            ...
            if "[" in cons:
                ...
                while (
                    vidx < conslen
                    and vt != eos_tokens[bspn_mode]
                    and "[" not in vt
                    and vt not in get_slot
                ):
            ...
            for turn_no, turn in enumerate(turns):
                ...
                for key in field:
                    ...
                    if key == "turn_domain":
                        v = " ".join(v)
                    entry[key] = (
                        decode_fn(v, eos=eos_syntax[key])
