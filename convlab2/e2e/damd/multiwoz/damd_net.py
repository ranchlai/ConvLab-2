...
def get_one_hot_input(x_input_np):
    ...
    input_t_onehot[:, :, 0] = 0.0  # <pad> to zero
...
class Attn(nn.Module):
    ...
    def score(self, hidden, encoder_outputs):
        ...
        energy = torch.tanh(
            self.attn(torch.cat([H, encoder_outputs], 2))
        )  # [B,T,2H]->[B,T,H]
...
class MultiLayerGRUwithLN(nn.Module):
    ...
    def forward(self, inputs, hidden=None):
        ...
        if hidden:
            hiddens = hidden.view(
                self.layer_num, self.bidirec, batch_size, self.hidden_size
            )
        for l in range(self.layer_num):
...
class Copy(nn.Module):
    ...
    def forward(self, enc_out_hs, dec_hs):
        ...
        raw_cp_score = torch.einsum(
            "beh,bdh->bde", raw_cp_score, dec_hs
        )  # [B, Tdec, Tenc]
...
def get_final_scores(
    ...
    logsoftmax = torch.nn.LogSoftmax(dim=2)
    normalized_scores = logsoftmax(
        torch.cat(raw_scores, dim=2)
    )  # [B,Tdec, V+V+Tenc1+V+Tenc2+...]
    ...
    c_to_g_scores = []
    for i in range(1, len(cum_idx) - 1):
        cps = normalized_scores[
            :, :, cum_idx[i] : cum_idx[i + 1]
        ]  # [B, Tdec, V+Tenc_i]
        # print('normalized_cp_scores:' , cps.cpu().detach().numpy()[0,:5, 0:40])
        c_to_g_scores.append(cps[:, :, :V])
        cp_score = cps[:, :, V:]
...
class DomainSpanDecoder(nn.Module):
    def __init__(self, embedding, vocab_size_oov, Wgen=None, dropout=0.0):
        ...
        self.attn_pvresp = (
            self.attn_user if cfg.attn_param_share else Attn(cfg.hidden_size)
        )
        self.attn_pvdspn = (
            self.attn_user if cfg.attn_param_share else Attn(cfg.hidden_size)
        )
    ...
    def forward(
        ...
        if first_step:
            self.mask_user = (inputs["user"] == 0).unsqueeze(
                1
            )  # .to(dec_last_w.device)     # [B,1,T]
            self.mask_pvresp = (inputs["pv_resp"] == 0).unsqueeze(
                1
            )  # .to(dec_last_w.device)     # [B,1,T]
            self.mask_pvdspn = (inputs["pv_dspn"] == 0).unsqueeze(
                1
            )  # .to(dec_last_w.device)     # [B,1,T]
        ...
        if not first_turn:
            context_pvresp = self.attn_pvresp(
                dec_last_h, hidden_states["resp"], self.mask_pvresp
            )
            context_pvdspn = self.attn_pvdspn(
                dec_last_h, hidden_states["dspn"], self.mask_pvdspn
            )
        else:
            batch_size = inputs["user"].size(0)
            context_pvresp = cuda_(
                torch.zeros(batch_size, 1, cfg.hidden_size)
            )  # .to(context_user.device)
            context_pvdspn = cuda_(
                torch.zeros(batch_size, 1, cfg.hidden_size)
            )  # .to(context_user.device)
...
class BeliefSpanDecoder(nn.Module):
    ...
    def forward(
        ...
        if first_step:
            ...
            self.mask_pvbspn = (inputs["pv_" + self.bspn_mode] == 0).unsqueeze(
                1
            )  # .to(dec_last_w.device)     # [B,1,T]
            # print('masks:', self.mask_user.device, self.mask_pvresp.device, self.mask_pvbspn.device)
        if mode == "test" and not first_step:
...
class ActSpanDecoder(nn.Module):
    ...
    def forward(
        ...
        if first_step:
            self.mask_usdx = (inputs["usdx"] == 0).unsqueeze(
                1
            )  # .to(dec_last_w.device)     # [B,1,T]
            self.mask_pvaspn = (inputs["pv_aspn"] == 0).unsqueeze(
                1
            )  # .to(dec_last_w.device)     # [B,1,T]
            if cfg.enable_bspn:
                self.mask_bspn = (inputs[cfg.bspn_mode] == 0).unsqueeze(
                    1
                )  # .to(dec_last_w.device)     # [B,1,T]
            if cfg.enable_dspn:
                self.mask_dspn = (inputs["dspn"] == 0).unsqueeze(
                    1
                )  # .to(dec_last_w.device)     # [B,1,T]
        if mode == "test" and not first_step:
            self.mask_pvaspn = (inputs["pv_aspn"] == 0).unsqueeze(
        ...
        if cfg.enable_dspn:
            if bidx is None:
                context_dspn = self.attn_dspn(
                    dec_last_h, hidden_states["dspn"], self.mask_dspn
                )
            else:
                context_dspn = self.attn_dspn(
                    dec_last_h,
        ...
        if cfg.use_pvaspn:
            if not first_turn:
                ...
                if bidx is None:
                    context_pvaspn = cuda_(
                        torch.zeros(inputs["user"].size(0), 1, cfg.hidden_size)
                    )
                else:
    ...
    def get_probs(
        ...
        if cfg.enable_dspn:
            if bidx is None:
                ...
                raw_cp_score_dspn = self.cp_dspn(
                    hidden_states["dspn"][bidx], dec_hs
                )  # [B,Tb]
                raw_cp_score_dspn.masked_fill_(
                    self.mask_dspn[bidx].repeat(1, Tdec, 1), -1e20
                )
                raw_scores.append(raw_cp_score_dspn)
        ...
        if not first_turn and cfg.use_pvaspn:
            if bidx is None:
                raw_cp_score_aspn = self.cp_pvaspn(
                    hidden_states["aspn"], dec_hs
                )  # [B,Ta]
                raw_cp_score_aspn.masked_fill_(
                    self.mask_pvaspn.repeat(1, Tdec, 1), -1e20
                )
                ...
                raw_cp_score_aspn = self.cp_pvaspn(
                    hidden_states["aspn"][bidx], dec_hs
                )  # [B,Ta]
                raw_cp_score_aspn.masked_fill_(
                    self.mask_pvaspn[bidx].repeat(1, Tdec, 1), -1e20
                )
                raw_scores.append(raw_cp_score_aspn)
...
class ResponseDecoder(nn.Module):
    def __init__(self, embedding, vocab_size_oov, Wgen=None, dropout=0.0):
        ...
        if cfg.enable_bspn:
            self.cp_bspn = (
                self.cp_usdx if cfg.copy_param_share else Copy(cfg.hidden_size)
            )
        if cfg.enable_aspn:
            self.cp_aspn = (
                self.cp_usdx if cfg.copy_param_share else Copy(cfg.hidden_size)
    ...
    def forward(
        ...
        if first_step:
            ...
            if cfg.enable_aspn:
                self.mask_aspn = (inputs["aspn"] == 0).unsqueeze(
                    1
                )  # .to(dec_last_w.device)     # [B,1,T]
        if mode == "test" and not first_step:
            if cfg.enable_bspn:
        ...
        if cfg.enable_bspn:
            ...
            gru_input.append(context_bspn)
        if cfg.enable_aspn:
            context_aspn = self.attn_aspn(
                dec_last_h, hidden_states["aspn"], self.mask_aspn
            )
...
class DAMD(nn.Module):
    ...
    def supervised_loss(self, inputs, probs):
        ...
        for name, prob in probs.items():
            ...
            # print(pred[0, :50])
            if name != "resp" or cfg.label_smoothing == 0.0:
                ...
                label = label_smoothing(
                    inputs[name + "_4loss"], self.label_smth, self.vsize_oov
                )
    ...
    def train_forward(self, inputs, hidden_states, first_turn):
        ...
        def train_decode(name, init_hidden, hidden_states, probs, bidx=None):
            ...
            decode_step = (
                inputs[name].size(1)
                if bidx is None
                else inputs["aspn_aug_batch"].size(1)
            )
            hiddens = []
            for t in range(decode_step):
                ...
                if bidx is None:
                    ...
                    assert (
                        name == "aspn"
                    ), "only act span decoder support batch idx selection"
                    dec_last_h = self.decoders[name](
                        inputs,
                        hidden_states,
                        dec_last_w,
            ...
            if bidx is None:
                ...
                probs = self.decoders[name].get_probs(
                    inputs, hidden_states, dec_hs, first_turn, bidx=bidx
                )
        ...
        if cfg.enable_dspn:
            ...
            hidden_states, probs = train_decode(
                "dspn", usdx_enc_last_h, hidden_states, probs
            )
        ...
        if cfg.enable_bspn:
            ...
            hidden_states, probs = train_decode(
                cfg.bspn_mode, init_hidden, hidden_states, probs
            )
        ...
        if cfg.enable_aspn:
            ...
            hidden_states, probs = train_decode(
                "aspn", usdx_enc_last_h, hidden_states, probs
            )

        hidden_states, probs = train_decode(
            "resp", usdx_enc_last_h, hidden_states, probs
        )
    ...
    def test_forward(self, inputs, hs, first_turn):
        ...
        if cfg.enable_dst and cfg.bspn_mode == "bsdx":
            ...
            hs, decoded = self.greedy_decode(
                "bspn", user_enc_last_h, first_turn, inputs, hs, decoded
            )
        ...
        if cfg.enable_dspn:
            ...
            hs["dspn"] = dspn_enc
            hs, decoded = self.greedy_decode(
                "dspn", usdx_enc_last_h, first_turn, inputs, hs, decoded
            )
        ...
        if cfg.enable_bspn:
            ...
            hs, decoded = self.greedy_decode(
                cfg.bspn_mode, init_hidden, first_turn, inputs, hs, decoded
            )
            ...
            if not cfg.use_true_db_pointer and "bspn" in decoded:
                for bi, bspn_list in enumerate(decoded["bspn"]):
                    ...
                    for domain, info_slots in constraint_dict.items():
                        if info_slots:
                            if domain not in self.reader.constraint_dict:
                                ...
                                self.reader.constraint_dict[domain]
                    ...
                    if len(turn_domain) == 2:
                        if (
                            len(self.reader.turn_domain) == 1
                            and self.reader.turn_domain[0] == turn_domain[1]
                        ):
                    ...
                    bkvec = self.reader.db.addBookingPointer(
                        constraint_dict.get(match_dom, []),
                        match_dom,
                        self.reader.book_state,
                    )
                    if bkvec == [0, 1]:
    ...
    def addActSelection(self):
        ...
        self.act_selection = ActSelectionModel(
            cfg.hidden_size, cfg.max_span_length, cfg.nbest
        )
    ...
    def greedy_decode(
        self, name, init_hidden, first_turn, inputs, hidden_states, decoded
    ):
        ...
        for t in range(max_len):
            ...
            prob_turn = self.decoders[name].get_probs(
                inputs, hidden_states, dec_hs, first_turn
            )  # [B,1,V_oov]
            hiddens.append(dec_last_h)
        ...
        hidden_states[name] = torch.cat(hiddens, dim=0).transpose(
            0, 1
        )  # [1,B,H] ---> [B,T,H]
    ...
    def beam_decode(
        ...
        for bidx in range(batch_size):
            ...
            while True:
                ...
                prob_turn = self.decoders[name].get_probs(
                    inputs, hidden_states, dec_h, first_turn, bidx=[bidx]
                )  # [B,1,V_oov]
        ...
        for s in range(cfg.nbest):
            ...
            for t in range(max_len):
                ...
                if cfg.aspn_decode_mode == "topk_sampling":
                    ...
                    logprobs, topk_words = torch.topk(
                        prob_turn.squeeze(1), 55
                    )  # 55 is enough for valid aspn tokens
                ...
                decode_idx.append(
                    dec_last_w.view(-1).clone()
                )  # list of [B] of length T
                dec_last_w[dec_last_w >= self.vocab_size] = 2

            decoded_np = torch.stack(decode_idx, dim=1)  # [B, T]
            hiddens_batch.append(
                torch.cat(hiddens, dim=0).transpose(0, 1)
            )  # list of [B, T, H] of length nbest
        ...
        if cfg.act_selection_scheme == "high_test_act_f1":
            ...
            for b in range(batch_size):
                ...
                for i in range(self.nbest):
                    decode_str = self.reader.vocab.sentence_decode(
                        decoded["aspn"][i][b], eos="<eos_a>"
                    )
