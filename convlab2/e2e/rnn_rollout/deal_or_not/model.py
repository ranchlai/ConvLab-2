...
class DealornotAgent(RNNRolloutAgent):
    ...
    def __init__(
        ...
        model = RnnModel(
            corpus.word_dict,
            corpus.item_dict_old,
            corpus.context_dict,
            corpus.count_dict,
            args,
        )
        state_dict = utils.load_model(
            os.path.join(self.config_path, args.model_file)
        )  # RnnModel
        ...
        super(DealornotAgent, self).__init__(
            model, sel_model, args, name, train, diverse, max_total_len
        )
    ...
    def auto_download(self):
        ...
        if os.path.exists(
            os.path.join(self.config_path, "model/rnn_model_state_dict.th")
        ) and os.path.exists(
            os.path.join(self.config_path, "selection_model_state_dict.th")
        ):
            return
