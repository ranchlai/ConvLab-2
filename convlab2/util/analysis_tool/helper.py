...
class DomainRecorder:
    ...
    def get_info(self):
        ...
        return (
            self.tot_num,
            self.suc_num / self.tot_num,
            self.pre / self.tot_num,
            self.rec / self.tot_num,
            self.f1 / self.tot_num,
            cycle_tot,
            tmp,
            self.turn_num / self.tot_num,
        )
    ...
    def format_result(self):
        ...
        for da in self.failed_nlu_da_sys:
            tmp = [
                "-".join(da),
                self.failed_nlu_da_sys[da][0],
                [
                    ("-".join(fda), self.failed_nlu_da_sys[da][1][fda])
                    for fda in self.failed_nlu_da_sys[da][1]
                ],
            ]
        ...
        for da in self.failed_nlu_da_usr:
            tmp = [
                "-".join(da),
                self.failed_nlu_da_usr[da][0],
                [
                    ("-".join(fda), self.failed_nlu_da_usr[da][1][fda])
                    for fda in self.failed_nlu_da_usr[da][1]
                ],
            ]
...
class Reporter:
    ...
    def report(self, com, suc, pre, rec, f1, turn_suc, turn):
        ...
        writer = HTMLWriter(
            "results/%s/report_%s.html" % (self.modelname, self.dataset)
        )
