...
class Analyzer:
    ...
    def comprehensive_analyze(self, sys_agent, model_name, total_dialog=100):
        ...
        for j in tqdm(range(total_dialog), desc="dialogue"):
                    ...
                    for da1 in sess.user_agent.get_out_da():
                        for da2 in sess.sys_agent.get_in_da():
                            if (
                                ...
                                and da1 is not None
                                and da2 is not None
                                and (da1, da2) not in failed_da_sys
                            ):
                                failed_da_sys.append((da1, da2))

                if (
                    isinstance(last_sys_da, list)
                    and last_sys_da is not None
                    and last_sys_da != []
                    and sess.user_agent.get_in_da() != last_sys_da
                ):
                    for da1 in last_sys_da:
                        for da2 in sess.user_agent.get_in_da():
                            if (
                                ...
                                and da1 is not None
                                and da2 is not None
                                and (da1, da2) not in failed_da_usr
                            ):
                                failed_da_usr.append((da1, da2))

                last_sys_da = (
                    sess.sys_agent.get_out_da()
                    if hasattr(sess.sys_agent, "get_out_da")
                    else None
                )
            ...
            if len(sess.evaluator.goal) > 0:
                ...
                num_domains_satisfying_constraints += (
                    len(sess.evaluator.goal) * percentage
                )
            ...
            if (j + 1) % 100 == 0:
                ...
                logging.info(
                    ...
                    % (
                        1
                        if num_domains == 0
                        else (num_domains_satisfying_constraints / num_domains)
                    )
                )
                logging.info(
        ...
        reporter.report(
            ...
            suc_num / total_dialog,
            np.mean(precision),
            np.mean(recall),
            np.mean(f1),
            tmp,
            turn_num / total_dialog,
        )
