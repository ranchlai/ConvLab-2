...
class GoalGenerator:
    ...
    def _build_goal_model(self):
        ...
        if "info" in ind_slot_value_cnt:
            for slot in ind_slot_value_cnt["info"]:
                ...
                self.ind_slot_dist["info"][slot] = sum(
                    ind_slot_value_cnt["info"][slot].values()
                ) / len(dialogs)
                slot_total = sum(ind_slot_value_cnt["info"][slot].values())
                for val in self.ind_slot_value_dist["info"][slot]:
                    self.ind_slot_value_dist["info"][slot][val] = (
                        ind_slot_value_cnt["info"][slot][val] / slot_total
                    )
    ...
    def _get_restaurant_goal(self):
        ...
        while True:
            ...
            if (
                "info" in domain_goal
                and len(self.db.query(domain_goal["info"].items())) == 0
            ):
                num_trial = 0
                while num_trial < 100:
    ...
    def _adjust_info(self, info):
        ...
        adjusted_info[slot] = random.choice(
            list(self.ind_slot_value_dist["info"][slot].keys())
        )
