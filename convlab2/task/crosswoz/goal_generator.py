...
class SingleDomainGenerator:
    def __init__(self, database, domain_index=None):
        ...
        self.generators = [
            self.attraction_generator,
            self.restaurant_generator,
            self.hotel_generator,
        ]
        if domain_index:
...
def domain_count(all_goal_list):
    ...
    for domain in result:
        if result[domain]["每个目标平均小目标数"]:
            result[domain]["约束条件"] = (
                sum(result[domain]["约束条件"]) / result[domain]["每个目标平均小目标数"]
            )
            result[domain]["需求信息"] = (
                sum(result[domain]["需求信息"]) / result[domain]["每个目标平均小目标数"]
            )
        else:
...
def total_count(all_goal_list):
    ...
    result = {
        "单领域目标生成": {"先验概率": len(split_data[0]) / len(all_goal_list)},
        "多领域单独": {"先验概率": len(split_data[1]) / len(all_goal_list)},
        "多领域可跨领域": {"先验概率": len(split_data[2]) / len(all_goal_list)},
    }
    index = 0
