...
class MILU(Model):
    ...
    def __init__(
        ...
        nongeneral_intent_weight: float = 5.0,
        ...
        projection_input_dim = (
            ...
            else self.encoder.get_output_dim()
        )
        ...
        self.tag_projection_layer = TimeDistributed(
            Linear(tag_projection_input_dim, self.num_tags)
        )
    ...
    def forward(
        ...
        **kwargs
        ...
        if (
            ...
            or self.context_for_tag
            or self.attention_for_intent
            or self.attention_for_tag
        ):
            ...
            encoded_context_summary = util.get_final_encoder_states(
                encoded_context, context_mask, self.encoder.is_bidirectional()
            )
        ...
        intent_encoded_text = (
            self.intent_encoder(encoded_text, mask)
        ...
        is_bidirectional = (
            self.intent_encoder.is_bidirectional()
            if self.intent_encoder
            else self.encoder.is_bidirectional()
        )
        ...
        if self.context_for_intent:
            encoded_summary = torch.cat(
                [encoded_summary, encoded_context_summary], dim=-1
            )
            ...
            tag_encoded_text = torch.cat(
                [
                    ...
                    encoded_context_summary.unsqueeze(dim=1).expand(
                        ...
                        encoded_context_summary.size(1),
                    ),
                ],
                dim=-1,
            )
        ...
        if tags is not None:
            if self.crf is not None:
                ...
                class_probabilities = sequence_logits * 0.0
        ...
        if tags is not None and metadata:
            self.decode(output)
            self._dai_f1_metric(
                output["dialog_act"], [x["dialog_act"] for x in metadata]
            )
            rewards = (
                self.get_rewards(
                    output["dialog_act"], [x["dialog_act"] for x in metadata]
        ...
        if intents is not None:
            output["loss"] += torch.mean(
                self.intent_loss(intent_logits, intents.float())
            )
    ...
    def decode(self, output_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        output_dict["tags"] = [
            [
                self.vocab.get_token_from_index(
                    tag, namespace=self.sequence_label_namespace
                )
        ...
        output_dict["intents"] = [
            [
                self.vocab.get_token_from_index(
                    intent[0], namespace=self.intent_label_namespace
                )
