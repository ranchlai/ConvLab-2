...
def preprocess(mode):
        ...
        for dialog in data[key]:
            ...
            for turn in dialog["dial"]:
                if mode == "usr" or mode == "all":
                    tokens, tags, intents, new_das = das2tags(
                        turn["usr"]["transcript"], turn["usr"]["dialog_act"]
                    )
                ...
                if mode == "sys" or mode == "all":
                    tokens, tags, intents, new_das = das2tags(
                        turn["sys"]["sent"], turn["sys"]["dialog_act"]
                    )
    ...
    json.dump(
        all_intent,
        open(os.path.join(processed_data_dir, "intent_vocab.json"), "w"),
        indent=2,
    )
    json.dump(
        all_tag,
