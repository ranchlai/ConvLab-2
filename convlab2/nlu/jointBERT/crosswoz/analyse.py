...
if __name__ == "__main__":
    ...
    for intent in intents:
        intent_predict_golden[intent] = [
            float("%.2f" % (x * 100))
            for x in calculateF1(predict_golden, intent=intent)
        ]
    for domain in domains:
        domain_predict_golden[domain] = [
            float("%.2f" % (x * 100))
            for x in calculateF1(predict_golden, domain=domain)
        ]
