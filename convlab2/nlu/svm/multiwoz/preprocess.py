...
if __name__ == "__main__":
    ...
    for no, sess in data["train"].items():
        for i, turn in enumerate(sess["log"]):
            ...
            for da, svs in turn["dialog_act"].items():
                if "Request" in da:
                    requestable_slots.extend(
                        [da.split("-")[0] + "-" + s for s, v in svs]
                    )
                else:
    ...
    informable_onto["Stars"] = slot2all_value("stars") + [
        "zero",
        "one",
        "two",
        "three",
    ...
    informable_onto["Stay"] = list([str(i) for i in range(10)]) + [
        "zero",
        "one",
        "two",
        "three",
    ...
    informable_onto["Choice"] = list([str(i) for i in range(20)]) + [
        "zero",
        "one",
        "two",
        "three",
    ...
    informable_onto["Car"] = [
        i + " " + j
        for i in ["black", "white", "red", "yellow", "blue", "grey"]
        for j in [
            "toyota",
            "skoda",
            "bmw",
    ...
    informable_onto["People"] = list([str(i) for i in range(10)]) + [
        "zero",
        "one",
        "two",
        "three",
            ...
            for s, v in sv.items():
                ...
                if s not in slots_enumerated:
                    ...
                    for i, c in dict(v_cnt).items():
                        if (
                            c > 0
                            and i in informable_onto[s]
                            or i.lower() in informable_onto[s]
                        ):
    ...
    json.dump(
        ontology_multiwoz,
        open("configs/ontology_multiwoz_{}.json".format(mode), "w"),
        indent=4,
    )
