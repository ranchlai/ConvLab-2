...
class nbest(object):
    ...
    def calculate(self, log_turn, log_input_key="batch"):
        ...
        for p, asr_hyp in asr_hyps:
            these_ngrams = get_ngrams(
                asr_hyp.lower(), self.max_length, skip_ngrams=self.skip_ngrams
            )
            ...
            for ngram, score in final_ngrams:
                if gvalue.value is not None:
                    if gvalue.value.lower() in ngram:
                        new_ngram = ngram.replace(
                            gvalue.value.lower(), "<generic_value>"
                        )
    ...
    def tuple_calculate(self, this_tuple, log_turn, log_input_key="batch"):
        ...
        if Tuples.is_generic(this_tuple[-1]):
            ...
            for ngram in final_ngrams:
                new_ngram = cn_ngram_replaced(
                    ngram, gvalue.value.lower(), "<generic_value>"
                )
                if new_ngram != False:
...
if __name__ == "__main__":
    cn = [
        ...
        {
            "arcs": [
                {"word": "there", "score": -math.log(2)},
                {"word": "!null", "score": -math.log(2)},
            ]
        },
