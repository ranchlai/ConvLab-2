...
from convlab2.dst.sumbt.multiwoz.convert_to_glue_format import (
...
class SUMBTTracker(DST):
    ...
    def train(self, load_model=False, model_path=None):
        ...
        for slot_idx, labels in zip(slot_token_ids, self.label_list):
            ...
            token_ids, lens = get_label_embedding(
                labels, args.max_label_length, self.tokenizer, DEVICE
            )
        ...
        with open(
            ...
            f.write(
                ...
                % (
                    ...
                    (
                        accuracies["slot_rest"] / accuracies["num_slot_rest"]
                    ).item(),
                )
            )
...
def test_update():
    ...
    sumbt_tracker.state["history"].append(
        [
            "user",
    ...
    sumbt_tracker.state["history"].append(
        ["user", "it doesn't matter. I don't care"]
    )
...
def test_update_bak():
    ...
    sumbt_tracker.state["history"][-1].append(
        "Friday and Can you book it for me and get a reference number ?"
    )
    end = timer()
    print(end - start)
