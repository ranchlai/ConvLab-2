    ...
    if subtask == "multiwoz":
        ...
        for dialog_id, dialog in test_data.items():
            ...
            if correct_name_label:
                selected_results = {
                    domain_name: [] for domain_name in turns[1]["sys_state_init"]
                }
            for i in range(0, len(turns), 2):
                ...
                for domain_name, state in turns[i + 1]["sys_state_init"].items():
                    if correct_name_label:
                        ...
                        state_change = (
                            i == 0 or state != turns[i - 1]["sys_state"][domain_name]
                        )
...
def eval_states(gt, pred, subtask):
    ...
    errors = [
        ["dialog id", "turn id", "domain name", "slot name", "ground truth", "predict"]
    ]
