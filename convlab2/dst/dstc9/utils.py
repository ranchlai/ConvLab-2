...
def prepare_data(
    subtask, split, data_root=DATA_ROOT, correct_name_label=False
):
...
def eval_states(gt, pred, subtask):
    ...
    for dialog_id, gt_states in gt.items():
        ...
        if len(gt_states) != len(pred_states):
            return exception(
                f"turns number incorrect, {len(gt_states)} expected, {len(pred_states)} found",
                dialog_id=dialog_id,
            )

        for turn_id, (gt_state, pred_state) in enumerate(
            zip(gt_states, pred_states)
        ):
            joint_tot += 1
            turn_result = True
            for domain_name, gt_domain in gt_state.items():
                ...
                for slot_name, gt_value in gt_domain.items():
                    ...
                    if (
                        gt_value == pred_value
                        or isinstance(gt_value, list)
                        and pred_value in gt_value
                    ):
                        slot_acc += 1
                        if gt_value:
                            tp += 1
...
def dump_result(model_dir, filename, result, errors=None, pred=None):
    ...
    if pred:
        json.dump(
            pred,
            open(os.path.join(output_dir, "pred.json"), "w"),
            indent=4,
            ensure_ascii=False,
        )
