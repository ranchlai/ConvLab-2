...
parser.add_argument("-log", default="temp", type=str, help="log directory")
...
print(len(tgt_vocab))
trainloader = dataloader.get_loader(
...
def train_and_eval(epoch):
    ...
    for (
        ...
        for i in range(src1.shape[1]):
            ...
            loss, sloss, vloss, num, snum, vnum = model(
                src1s,
                src1_lens,
                src2s,
                src2_lens,
        ...
        if updates % config.log_interval == 0:
            logging(
                "time: %6.3f, epoch: %3d, updates: %8d, train loss: %6.5f, train sloss: %6.5f, train vloss: %6.5f\n"
                % (
                    time.time() - start_time,
                    epoch,
                    updates,
                    total_loss / report_total,
                    total_sloss / report_total,
                    total_vloss / report_total,
                )
            )
            total_loss = 0
            total_sloss = 0
...
def eval(epoch):
    ...
    with torch.no_grad():
        for (
            ...
            for x, xv, xvv, y, yv, yvv in zip(
                samples, ssamples, vsamples, tgt[0], tgtv[0], tgtpv[0]
            ):
                # each turn
                x = x.data.cpu()
                y = y.data.cpu()
    ...
    jacc = []
    jaacc = []
    for p, l, jp, jl, jap, jal in zip(
        preds, labels, joint_preds, joint_labels, joint_allps, joint_alls
    ):
        acc.append(p == l)
        jacc.append(jp == jl)
