...
class ComerTracker(DST):
    def __init__(
        self,
        model_file="https://huggingface.co/ConvLab/ConvLab-2_models/resolve/main/comer.zip",
        ...
        parser.add_argument(
            "-restore",
            default="data/log/norml_mwNestedNOND128NOsaA1FNN/checkpoint.pt",
            type=str,
        ...
        if (
            ...
            and os.path.exists(
                os.path.join(
                    self.root_path,
                    "data/log/norml_mwNestedNOND128NOsaA1FNN/checkpoint.pt",
                )
            )
        ):
            pass
        ...
        self.sl_dict = json.load(
            open(os.path.join(self.root_path, "data/mwoz2_sl.dict"))
        )
        self.dm_dict = json.load(
        ...
        pretrain_embed = {}
        pretrain_embed["slot"] = torch.load(
        ...
        if use_cuda:
            bmodel.to("cuda")
        self.model = getattr(models, opt.model)(
            config,
            self.vocab,
            self.vocab,
            use_cuda,
    ...
    def model_predict(self, history):
        ...
        for i, s in enumerate(history):
            ta = (
                ["[CLS]", "system", ":"]
                + self.tokenizer.tokenize(normalize(s[0], False))
                + ["[SEP]"]
            )
            tb = (
                ["[CLS]", "user", ":"]
                + self.tokenizer.tokenize(normalize(s[1], False))
                + ["[SEP]"]
            )
        ...
        for (
            ...
            samples, ssamples, vsamples, _ = self.model.sample(
                src1, src1_len, src2, src2_len, None, None, None, None
            )
