...
class rnn_encoder(nn.Module):
    ...
    def forward(self, x1, x1_len, bert, app_time=None, embedding=None):
        ...
        if bert:
            with torch.no_grad():
                max_len = len(x1)
                input_mask = torch.arange(max_len).cuda().expand(
                    len(x1_len), max_len
                ) < x1_len.unsqueeze(1)
                xt = x1.transpose(0, 1)
                encoded_layers, _ = embedding(
                    xt, token_type_ids=None, attention_mask=input_mask
                )
        ...
        if not self.config.bidirec:
            ...
            h = (
                h.transpose(0, 1)
                .contiguous()
                .view(batch_size, -1, 2 * self.config.encoder_hidden_size)
            )
            c = (
                c.transpose(0, 1)
                .contiguous()
...
class rnn_decoder(nn.Module):
    ...
    def forward(
        ...
        for emb in embs.split(1):
            ...
            output = self.re1(
                self.linear_out(torch.cat([cout, cout1, cout2, outputt3], 1))
            )
    ...
    def sample(self, input, init_state, ifslot, contexts, story, lengths, slot=None):
        ...
        for _ in range(20):  # max_gen_lengtuh
            ...
            (
                ...
                inputs[-1], state, contexts, ifslot, story, lengths, slot
            )
