...
class seq2seq(nn.Module):
    def __init__(
        ...
        self.encoder = models.rnn_encoder(
            config,
        )  # tgt_vocab_size,False, embedding=slot_embedding)
    ...
    def forward(
        ...
        b_out, b_state = self.encoder(
            src3, src3_len, False, embedding=self.slot_embedding
        )
        user_out, user_state = self.encoder(
            src1, src1_len, True, embedding=self.src_embedding
        )

        sys_out, sys_state = self.encoder(
            src2, src2_len, True, embedding=self.src_embedding
        )

        # combine state
        state = (
            (b_state[0] + user_state[0] + sys_state[0]) / 3,
            (b_state[1] + user_state[1] + sys_state[1]) / 3,
        )

        lengths = (
            sequence_mask(src3_len, src3.size(0)),
            sequence_mask(src1_len, src1.size(0)),
            sequence_mask(src2_len, src2.size(0)),
        )
        story = torch.cat((src3, src2, src1), dim=0).transpose(0, 1)
        # decoder
        outputs, loss, num = self.decoder(
            tgt,
            state,
            False,
            (
        ...
        for j, d in enumerate(outputs):  # tgt[i].split(1):
            ...
            soutput, sloss, snum = self.decoder(
                tgtv[j + 1],
                state,
                True,
                (
            ...
            for k, s in enumerate(soutput):
                voutput, vloss, vnum = self.decoder(
                    tgtpv[j + 1, k + 1],
                    state,
                    True,
                    (
    ...
    def sample(
        ...
        for i in range(src1s.shape[1]):
            ...
            b_out, b_state = self.encoder(
                b_in, b_len, False, embedding=self.slot_embedding
            )
            ...
            lengths = (
                sequence_mask(b_len, b_in.size(0)),
                sequence_mask(src1_len, src1.size(0)),
                sequence_mask(src2_len, src2.size(0)),
            )
            ...
            for j, d in enumerate(final_outputs[0]):  # tgt[i].split(1):
                ...
                ssample_ids, final_state = self.decoder.sample(
                    bos,
                    state,
                    True,
                    (
                ...
                for k, s in enumerate(final_state[0]):  # tgt[i].split(1):
                    ...
                    vsample_ids, vfinal_state = self.decoder.sample(
                        bos,
                        state,
                        True,
                        (
