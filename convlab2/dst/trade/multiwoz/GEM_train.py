...
### LOAD DATA
args["data_ratio"] = 1
train_GEM, _, _, _, _, _, _, _ = prepare_data_seq(
    True, args["task"], False, batch_size=64
)
...
try:
    for epoch in range(100):
        ...
        for i, data in pbar:
            ...
            model.train_batch(
                data, int(args["clip"]), SLOTS_LIST_single[1], reset=(i == 0)
            )
            model.loss_grad.backward()

            store_grad(model.parameters, grads, grad_dims, task_id=n_tasks - 1)
            indx = torch.cuda.LongTensor(
                [j for j in range(n_tasks - 1)]
            )  # if USE_CUDA else torch.LongTensor([0])
            dotp = torch.mm(
                grads[:, n_tasks - 1].unsqueeze(0), grads.index_select(1, indx)
            )
            if (dotp < 0).sum() != 0:
                project2cone2(
                    grads[:, n_tasks - 1].unsqueeze(1),
        ...
        if (epoch + 1) % int(args["evalp"]) == 0:
            acc = model.evaluate(
                dev_single, avg_best, SLOTS_LIST_single[2], args["earlyStop"]
            )
