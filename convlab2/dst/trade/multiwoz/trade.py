...
class MultiWOZTRADE(TRADE, nn.Module):
    def __init__(self, argpath=None, mode="en", training=False):
        ...
        (
            ...
            gating_dict,
            max_word,
        ) = prepare_data_seq(False, "dst", False, batch_size=4)
    ...
    def download_data(
        self,
        data_url="https://huggingface.co/ConvLab/ConvLab-2_models/resolve/main/trade_multiwoz_data.zip",
    ):
        """Automatically download the pretrained model and necessary data."""
        if os.path.exists(
    ...
    def update(self, user_act):
        ...
        context = " ; ".join(
            [item[1].strip().lower() for item in self.state["history"]]
        ).strip()
        sentence = self.state["history"][-1][1].strip().lower()
        _, _, dev, _, _, SLOTS_LIST, _, _ = self.reformat_input(
            False,
            "dst",
            False,
    ...
    def init_run_vars(self, training=False):
        ...
        self.ontology = json.load(
            open(root_path + "/data/multi-woz/MULTIWOZ2 2/ontology.json", "r")
        )
    ...
    def reformat_input(
        ...
        if not training:
            ...
            pair_test, test_max_len, slot_test = read_langs2(
                source_text,
                utterance,
                self.gating_dict,
                self.ALL_SLOTS,
