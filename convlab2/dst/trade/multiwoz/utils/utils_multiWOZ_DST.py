...
EXPERIMENT_DOMAINS = (
    ["餐馆", "地铁", "出租", "酒店", "景点"]
    if MODE == "cn"
    else ["hotel", "train", "restaurant", "attraction", "taxi"]
)
...
class Dataset(data.Dataset):
    ...
    def preprocess(self, sequence, word2idx):
        ...
        story = [
            word2idx[word] if word in word2idx else UNK_token
    ...
    def preprocess_memory(self, sequence, word2idx):
        ...
        for value in sequence:
            ...
            for wi, vw in enumerate(v.split()):
                idx = [
                    word2idx[word] if word in word2idx else UNK_token
                    for word in [d, s, "t{}".format(wi), vw]
                ]
                story.append(idx)
        story = torch.Tensor(story)
        return story
...
def collate_fn(data):
    ...
    def merge_memory(sequences):
        lengths = [len(seq) for seq in sequences]
        max_len = (
            1 if max(lengths) == 0 else max(lengths)
        )  # avoid the empty belief state issue
...
def prepare_data_seq(training, task="dst", sequicity=0, batch_size=100):
    ...
    if training:
        ...
        if os.path.exists(folder_name + lang_name) and os.path.exists(
            folder_name + mem_lang_name
        ):
    ...
    print(
        "[Train Set & Dev Set Slots]: Number is {} in total".format(
            str(len(SLOTS_LIST[2]))
        )
    )
    print(SLOTS_LIST[2])
...
def prepare_data_seq_cn(training, task="dst", sequicity=0, batch_size=100):
    ...
    if args["path"]:
        ...
        if data_version == "init":
            ...
            folder_name = (
                "save_cn_processed/{}-".format(args["decoder"])
                + args["addName"]
                + args["dataset"]
                + str(args["task"])
                + "/"
    ...
    if training:
        ...
        if not os.path.exists(emb_dump_path) and args["load_embedding"]:
            dump_pretrained_emb(
                lang.word2index, lang.index2word, emb_dump_path, mode="cn"
            )
    else:
        with open(folder_name + lang_name, "rb") as handle:
            lang = pickle.load(handle)
