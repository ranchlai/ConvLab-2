...
def collate_fn(data):
    def merge(sequences, max_len):
        ...
        if max_len:
            padded_seqs = torch.ones(
                len(sequences), max(lengths), MEM_TOKEN_SIZE
            ).long()
    ...
    data_form = {
        "dailID": item_info["dialID"],
        "turnID": item_info["turnID"],
        "content_arr": content_arr,
        "content_arr_len": content_arr_len,
        "bot_action_idx": bot_action_idx,
        "bot_action": item_info["bot_action"],
        "ent_query": ent_query,
        "ent_query_idx": ent_query_idx,
        "gold_response": item_info["gold_response"],
                ...
                if "\t" in line:
                    ...
                    for idx, key in enumerate(r.split(" ")):
                        if key in entity:
                            index = [
                                loc
                                for loc, val in enumerate(content_arr)
                                if (val[0] == key)
                            ]
                            if index:
                                index = max(index)
...
def prepare_data_seq(task, batch_size=100):
    ...
    if int(task) != 6:
        ...
        candid_file_path = (
            "data/dialog-bAbI-tasks/dialog-babi-task6-dstc2-candidates.txt"
        )
    ...
    pair_train, max_len_train = read_langs(
        file_train, ent, cand2DLidx, idx2candDL, max_line=None
    )
    pair_dev, max_len_dev = read_langs(
        file_dev, ent, cand2DLidx, idx2candDL, max_line=None
    )
    pair_test, max_len_test = read_langs(
        file_test, ent, cand2DLidx, idx2candDL, max_line=None
    )

    max_r_test_OOV = 0
    if int(task) != 6:
        pair_test_OOV, max_len_test_OOV = read_langs(
