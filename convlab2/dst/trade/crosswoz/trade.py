...
class CrossWOZTRADE(TRADE, nn.Module):
    def __init__(self, argpath=None, mode="cn"):
        ...
        BSZ = (
            int(args["batch"])
            if args["batch"]
            else int(directory[2].split("BSZ")[1].split("DR")[0])
        )
        ...
        (
            ...
            gating_dict,
            max_word,
        ) = prepare_data_seq_cn(False, "dst", False, batch_size=4)
        ...
        self.encoder = EncoderRNN(
            self.lang.n_words, hidden_size, self.dropout, mode=mode
        )
    ...
    def download_model(
        self,
        model_url="https://huggingface.co/ConvLab/ConvLab-2_models/resolve/main/trade_crosswoz_model.zip",
    ):
        """Automatically download the pretrained model and necessary data."""
        if os.path.exists(
            os.path.join(self.crosswoz_root, "model/TRADE-multiwozdst")
        ):
            return
        ...
        if not os.path.exists(
            os.path.join(model_dir, "trade_crosswoz_model.zip")
        ):
    ...
    def download_data(
        self,
        data_url="https://huggingface.co/ConvLab/ConvLab-2_models/resolve/main/trade_crosswoz_data.zip",
    ):
        """Automatically download the pretrained model and necessary data."""
        if os.path.exists(
            os.path.join(self.crosswoz_root, "data/crosswoz")
        ) and os.path.exists(
            os.path.join(self.crosswoz_root, "data/dev_dials.json")
        ):
            return
        ...
        if not os.path.exists(
            os.path.join(data_dir, "trade_crosswoz_data.zip")
        ):
    ...
    def train_batch(self, data, clip, slot_temp, reset=0):
        ...
        (
            ...
            words_point_out,
            words_class_out,
        ) = self.encode_and_decode(data, use_teacher_forcing, slot_temp)
    ...
    def evaluate(self, early_stop=None):
        ...
        if args["genSample"]:
            json.dump(
                all_prediction,
                open("all_prediction_{}.json".format(self.name), "w"),
                indent=4,
            )
    ...
    def update(self, user_act):
        if type(user_act) is not str:
            raise Exception(
                "Expected user_act to be <class 'str'> type, but get {}.".format(
                    type(user_act)
                )
            )
        ...
        context = " ; ".join(
            [item[1].strip() for item in self.state["history"]]
        ).strip()
        sentence = self.state["history"][-1][1].strip()
        _, _, dev, _, _, SLOTS_LIST, _, _ = prepare_data_seq_cn2(
            False,
            "dst",
            False,
        ...
        for j, data_dev in enumerate(dev):
            ...
            for bi in range(1):
                ...
                self.state["belief_state"] = self.reformat_belief_state(
                    predict_belief_bsz_ptr,
...
class EncoderRNN(nn.Module):
    def __init__(
        ...
        if args["load_embedding"]:
            if mode == "en":
                with open(
                    os.path.join("data/", "emb_en{}.json".format(vocab_size))
                ) as f:
                    E = json.load(f)
            else:
                with open(
