...
class TemplateNLG(NLG):
    def __init__(self, is_user, mode="manual"):
        ...
        self.auto_user_template = self.read_json(
            os.path.join(template_dir, "auto_user_template_nlg.json")
        )
        self.auto_system_template = self.read_json(
            os.path.join(template_dir, "auto_system_template_nlg.json")
        )
        self.manual_user_template = self.read_json(
            os.path.join(template_dir, "manual_user_template_nlg.json")
        )
        self.manual_system_template = self.read_json(
            os.path.join(template_dir, "manual_system_template_nlg.json")
        )
    ...
    def _auto_manual_generate(self, dialog_acts, temp_auto, temp_manual):
        ...
        for intent in ["nooffer", "inform", "request"]:
            ...
            sentence = self._generate_single_intent_auto(
                intent, dialog_acts[intent], temp_auto
            )
            if sentence == "None":
                sentence = self._generate_single_intent_manual(
                    intent, dialog_acts[intent], temp_manual
                )
    ...
    def _manual_generate(self, dialog_acts, template):
        ...
        for intent in ["nooffer", "inform", "request"]:
            ...
            sentences += " " + self._generate_single_intent_manual(
                intent, dialog_acts[intent], template
            )
