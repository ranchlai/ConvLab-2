...
class DatasetCam(object):
    """
    data container for cam dataset
    """
    ...
    def next_batch(self, data_type="train"):
        ...
        for dial_idx, turn_idx, text, meta in data:
            ...
            da_idx, sv_idx, featStr = self.getFeatIdx(meta)
            da_cond = [
                1 if i in da_idx else 0 for i in range(self.da_size)
            ]  # dial act condition
            sv_cond = [
                1 if i in sv_idx else 0 for i in range(self.sv_size)
            ]  # slot/value condition
            feats.append(da_cond + sv_cond)
        ...
        seq_pairs = sorted(
            zip(sentences, refs, feats, featStrs, sv_indexes),
    ...
    def getFeatIdx(self, meta):
        ...
        for da, slots in meta.items():
            ...
            for _slot in slots:  # e.g. ('Day', '1', 'Wednesday ')
                sv_idx.append(
                    self.cardinality.index(
                        "a-s-v:" + da + "-" + _slot[0] + "-" + _slot[1]
                    )
                    - self.dfs[1]
                )
                feat_container.append(da + "-" + _slot[0] + "-" + _slot[1])

        feat_container = sorted(
            feat_container
        )  # sort SVs across DAs to make sure universal order
        feat = "|".join(feat_container)

        return da_idx, sv_idx, feat


class SimpleDatasetCam(DatasetCam):
