...
import torch
from convlab2.nlg.sclstm.camrest.loader.dataset_cam import DatasetCam, SimpleDatasetCam
    ...
    for _das in das:
        ...
        for _feat in feat:
            if _feat in das_order:
                feat_count += 1
        _das = _das.replace("a-s-v:", "").lower().split("-")
        slot_tok = "@" + _das[0] + "-" + _das[1]
...
def get_slot_error(dataset, gens, refs, sv_indexes):
    ...
    for batch_idx in range(batch_size):
        for beam_idx in range(beam_size):
            felements = [
                dataset.cardinality[x + dataset.dfs[1]] for x in sv_indexes[batch_idx]
            ]

            # get slot error per sample(beam)
            total, redunt, miss = score(
                felements, gens[batch_idx][beam_idx], dataset.template
            )
...
def evaluate(config, dataset, model, data_type, beam_search, beam_size, batch_size):
    ...
    for i in range(dataset.n_batch[data_type]):
        (
            ...
            featStrs,
            sv_indexes,
        ) = dataset.next_batch(data_type=data_type)
    ...
    print(
        "{} Loss: {:.3f} | Slot error: {:.3f} | Time: {:.1f}".format(
            data_type, total_loss, se, time.time() - t
        )
    )
    print(
        "{} Loss: {:.3f} | Slot error: {:.3f} | Time: {:.1f}".format(
    ...
    print(
        "redunt: {}, miss: {}, total: {}".format(
            countAll["redunt"], countAll["miss"], countAll["total"]
        )
    )
    print(
        "redunt: {}, miss: {}, total: {}".format(
...
def read(config, args, mode):
    ...
    # get feat size
    d_size = dataset.da_size + dataset.sv_size  # len of 1-hot feat
    ...
    if mode == "train":
        print(
            "do not support re-train model, please make sure the model do not exist before training"
        )
...
def interact(config, args):
    ...
    example = {
        "Attraction-Inform": [["Choice", "many"], ["Area", "centre of town"]],
        "Attraction-Select": [
            ["Type", "church"],
            ["Type", " swimming"],
            ["Type", " park"],
    ...
    da_idx, sv_idx, featStr = dataset.getFeatIdx(example)
    da_cond = [
        1 if i in da_idx else 0 for i in range(dataset.da_size)
    ]  # dial act condition
    sv_cond = [
        1 if i in sv_idx else 0 for i in range(dataset.sv_size)
