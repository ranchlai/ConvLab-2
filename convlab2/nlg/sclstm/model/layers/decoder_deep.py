...
class DecoderDeep(nn.Module):
        ...
        for i in range(self.n_layer):
            ...
            if gen:
                ...
                output_hidden.append(
                    F.dropout(_hidden.clone(), p=self.dropout, training=True)
                )

        last_hidden, last_cell, last_dt = cur_hidden, cur_cell, cur_dt
        if not gen:
            for i in range(self.n_layer):
                last_hidden[i] = F.dropout(
                    last_hidden[i], p=self.dropout, training=True
                )
    ...
    def forward(
        ...
        for t in range(max_len):
            ...
            previous_out = self.logits2words(
                output, decoded_words, dataset, sample_size
            )
            vocab_t = (
                previous_out if gen else input_var[:, t, :]
    ...
    def get_onehot(self, word, dataset, batch_size=1):
        res = [
            [
                1 if index == dataset.word2index[word] else 0
    ...
    def beam_search(
        ...
        init_x = {
            ...
            "logProb": [0],
            "lastStates": {
                "hid": [init_hidden.clone() for _ in range(self.n_layer)],
        ...
        for batch_idx in range(batch_size):
            ...
            for t in range(max_len):
                cand_pool = (
                    []
                )  # pool keeps all candidates, max size: beam_size * beam_size
                # iter over history
                for beam_idx in range(beam_size):
                    ...
                    if last_word == "EOS_token":
                        ...
                        logProb, vocab_idx = dis.data.topk(
                            beam_size
                        )  # size: (beam_size)
                    ...
                    if (
                        t == 0
                    ):  # can only use 1 beam to extend cuz history of all beams is the same when t == 0
                        break
                ...
                if [x["history"][-1] for x in dec_words[batch_idx]] == [
                    "EOS_token" for _ in range(beam_size)
                ]:
                    break
