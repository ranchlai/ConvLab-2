        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            with open(file_path, encoding="utf-8") as f:
                if args.text_chunk:
                    ...
                    for line in f:
                        tokenized_text = tokenizer.convert_tokens_to_ids(
                            tokenizer.tokenize(line.strip() + " eos")
                        )
...
class TextSeqDataset(Dataset):
    def __init__(
        ...
        cached_features_file = os.path.join(
            directory,
            args.output_dir.replace(os.sep, "_")
            + "_cached_lm_"
            + str(block_size)
            + "_seqlen_"
            + str(max_seq)
            ...
            with open(file_path, encoding="utf-8") as f:
                for line in f:
                    ...
                    if args.use_tokenize:
                        tokenized_text = tokenizer.convert_tokens_to_ids(
                            tokenizer.tokenize(raw_str)
                        )
                        code_str_len = len(
                            tokenizer.convert_tokens_to_ids(
                                tokenizer.tokenize(code_str)
                            )
                        )
                    else:
                        tokenized_text = tokenizer.convert_tokens_to_ids(
                        ...
                        code_str_len = len(
                            tokenizer.convert_tokens_to_ids(code_str.split())
                        )
                    ...
                    if len(tokenized_text) < max_seq:
                        mask[-(max_seq - len(tokenized_text)) :] = [0] * (
                            max_seq - len(tokenized_text)
                        )
    ...
    def __getitem__(self, item):
        return (
            torch.tensor(self.examples[item]),
            torch.tensor(self.masks[item]),
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if (
                    ...
                    logger.info(
                        f"  EVALERR:  {(tr_loss - logging_loss)/float(args.logging_steps)}"
                    )
