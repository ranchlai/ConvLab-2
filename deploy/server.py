...
class ServerCtrl(object):
    ...
    def _turn(self, last_turn, model_map, history, input_module, data, modified_output):
        ...
        cur_cache = (
            ...
            else {name: None for name in MODULES}
        )
        ...
        nlu_model, dst_model, policy_model, nlg_model = self.get_module_models(
            model_map, cur_cache
        )
        ...
        for mod in MODULES:
            ...
            if temp_data is not None and model_map[mod] is not None:
                ...
                (model_ret[mod], new_cache[mod]) = self.modules[mod].run(
                    model_map[mod],
                    cur_cache[mod],
                    last_turn is None,
                    param,
    ...
    def _response_from_session(turns):
        ret = {
            ...
            "modified_model": [
                mod
                for (mod, val) in turns[-1]["modified_output"].items()
                if val is not None
            ]
            if turns
            else None,
