...
class ModuleCtrl(object):
    ...
    def __init__(self, module_name: str, infos: dict):
        assert module_name in self.mod2method.keys(), (
            "Unknow module name '%s'" % module_name
        )
        self.module_name = module_name
        self.method = self.mod2method[self.module_name]
        self.infos = copy.deepcopy(infos)
        self.models = {
            mid: ModelCtrl(mid, **self.infos[mid]) for mid in self.infos.keys()
        }
    ...
    def add_used_num(self, model_id: str):
        try:
            ...
            raise DeployError(
                "Unknow model id '%s'" % model_id, module=self.module_name
            )
    ...
    def run(self, model_id, cache, isfirst, params, input_nl, input_act):
        try:
            ret = self.models[model_id].run(
                self.method, cache, isfirst, params, input_nl, input_act
            )
        except TypeError:
            raise DeployError(
                "Unknow model id '%s'" % model_id, module=self.module_name
